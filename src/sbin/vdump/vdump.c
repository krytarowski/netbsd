...
typedef struct {
	...
	int             var_len_rec_idx;	/* index into data part of a
						 * block of */
	/* the current variabl length record's */
	/* header                             */
	int             var_len_rec_in_progress;	/* if TRUE we are
							 * processing a        */
	/* variable length record             */
	union old_record_header_t var_len_rec_hdr;	/* cur var length
							 * record's hdr    */
}               save_set_t;
...
/*
 * tty_mutex is used around informational messages to prevent and error
 * message from trampling the informational message in the middle of its
 * output.  And tty_mutex is also used to allow a user to respond to a single
 * question and not error message questions spit out by another thread.
 */
t_mutex         tty_mutex;	/* Global; used by libvdump routines */
t_mutex         abrt_mutex;	/* used to synch access to abort_wanted &
				 * Wt_mq vars */
...
static int
                Use_stdout = FALSE, Reliable = FALSE, Block_mode = FALSE, Compress_data = FALSE, Blocks = DEFAULT_BLOCKS, Verbose = FALSE, Xor_blocks = DEFAULT_XOR_BLOCKS, Update_file = TRUE, Dump_level = 9, Use_previous_format = FALSE, Backup_zeros = TRUE,	/* last minute change;
																																	 * see history */
                Tape_number = 1, Backup_subtree = FALSE;

static long
                Tape_Size = 0, Tape_density = 0, Total_dump_time = 0,	/* static time_t??? */
                Last_est_finish_time = 0, Last_kbytes_backed_up = 0, Alarm_time = 300,	/* want 5 min (300 secs) */
                Num_dirs_to_backup = 0, Num_files_to_backup = 0, Bytes_backed_up = 0, Bytes_to_backup = 0, Zeros_skipped = 0, Sparse_skipped = 0, Last_files = 0, Num_files_backed_up = 0, Num_dirs_backed_up = 0;

double
                Avg_kb_file_rate = 0.0;

static          time_t
                Last_start_time = 0, Time_amount = 0;
...
struct itimerval timeval, *timeret;
...
{
	/* argv variables */
	int             c, C = 0, D = 0, N = 0, P = 0, U = 0, V = 0, F = 0,
	                Z = 0, level = 0, f = 0, m = 0, q = 0, t = 0, r = 0,
	                u = 0, v = 0, w = 0, x = 0, b = 0, i = 0, h = 0,
	                T = 0, g = 0;
	int             B = 0, S = 0, d = 0, s = 0;	/* allow these options
							 * to be given */
	...
	int
	                j, done, backup_blocks = FALSE, help = FALSE, file_spec = FALSE, device = FALSE;
	...
	/*
	 * Create a mutex to guard console interaction among threads.  Don't
	 * want the user to have 2 questions to answer at one time.  Also
	 * create one to synchronize access to two global variables.
	 */
	...
	if ((argc <= 2) || posix_style(argc, argv)) {	/* if POSIX-style of
							 * args */

		/*
		 * Posix uses dash and the value follows immediately after
		 * the option.
		 */
		while ((c = getopt(argc, argv,
			...
			case 'd':
				/*
				 * single optional param for tape density
				 * (bytes per inch)
				 */
			...
			case 'N':
				/*
				 * no params required for no tape rewind when
				 * done
				 */
				N++;
				No_rewind = TRUE;
			...
			case 'U':
				/*
				 * no params required for no tape unload when
				 * done
				 */
				U++;
				No_unload = TRUE;
			...
			case 'P':
				/*
				 * no params required for converting to
				 * previous format
				 */
				P++;
			...
			case 'F':
				/*
				 * single param required for number of
				 * in-memory buffers
				 */
				F++;
		...
		/*
		 * don't need because doesn't have to have options if ( (B +
		 * C + D + N + P + U + level + f + m + q + t + r + u + v + x
		 * + b + F + S + T + h + d) < 0 ) { usage(); exit (1); }
		 */
	...
	*device_name_ptr = '\0';
	...
	if (!device) {
		...
		if (No_rewind) {
			strcpy(Device_name, DEFTAPE_NH);	/* "/dev/ntape/tape0_d1"
								 * in mtio.h */
		} else {
			strcpy(Device_name, DEFTAPE);	/* "/dev/tape/tape0_d1"
							 * in mtio.h */
		}
	...
	if (strcmp(Device_name, "-")) {
		...
		/*
		 * Backup 'device' (file) exists in the the fileset to backed
		 * up.  This causes the vdump to be twice the size it needs
		 * to be.  The fix will be to check to see if the backup
		 * device is in the fileset being backed up and if so warn
		 * the user.
		 * 
		 * One way we could check this is to see if the 'source' is a
		 * parent of the Device_name.  This fails in some cases.
		 * 
		 * Example Success: Source = /demo, Device_name =
		 * /demo/vdump.file
		 * 
		 * Example Failure: Source = /, Device_name = /dev/rmt0h
		 * 
		 * So we will need to check to see if the 'device_name' being
		 * backed up to is a file or a device.  If it is a device
		 * ignore this check.
		 * 
		 * Another case is were you have a fileset mounted on the
		 * fileset your backing up, and your backup file is on the
		 * second fileset
		 * 
		 * Example Failure: Source = /, Device_name =
		 * /usr/tmp/vdump.file
		 * 
		 * So we also need to check if the file in question is part of
		 * the original fileset, so a quick strcmp is out of the
		 * question.
		 * 
		 * What we can do is an lstat of the both the Source and the
		 * Device_name.  If the Device_name is a file type then check
		 * to to see if they are mounted on the the same device.
		 */
	 ...
	 */
	if ((S_ISLNK(stat_buf.st_mode)) && (realpath(source, pathname))) {	/* get real path of the
										 * source */
	...
	if (auto_Mount || link_Mount) {
		...
		if (Backup_subtree)	/* if they entered a -D perform a
					 * levl 0 dump. */
			Dump_level = 0;
	...
	/*
	 * To display the compression ratio in verbose mode when -C is on and
	 * there are bytes to compress. note: Compression is not always
	 * possible
	 */
...
start_backup(
	     ...
	     unsigned long *bytes_written	/* in/out - number of bytes
						 * written */
)
{
	int             status;
...
{
	int             xfer_size = 0, bytes_moved = 0, bytes_xferred,
	                bytes_compressed;
	...
	while (bytes2mv > 0) {
		...
		if (((DATA_FREE(Blk_size, ss->data_idx) == 0) &&
			...
			/*
			 * The block is empty OR we have to start a new block
			 * OR we can't span this rec across blocks and it
			 * won't fit in the current block, SO start a new
			 * block!
			 */
...
start_var_len_rec(
		  save_set_t * ss,	/* in - save-set descriptor */
		  union old_record_header_t * rec_hdr	/* in - ptr to var len
							 * rec's header */
)
{
 ...
 * 
 * After the directories and files are backed up, this routine records the file
 * system, the level of dump, and the date of completion into the dump
 * history file.  If backing up a MSFS file system, the history file is
 * /etc/vdumpdates; otherwise it is /etc/dumpdates.
 * 
 * If that level dump does not exist for the file system or if the
 * /etc/vdumpdates file was created, then it will append the new record onto
 * the file.  If that level dump does exist for the fs, it reads the contents
 * of /etc/vdumpdates into memory, overwrites a matching file system and dump
 * level record with a new date OR appends a new record onto the end, and
 * then writes the new memory contents to /etc/vdumpdates.  Level 0 is a
 * special case in that it will always read the contents into memory and
 * search for it.
 * 
...
update_dumpdate_file(char *path, time_t last_date, int *found_exact_level)
{
	...
	int             ret;
	int             created_file = 0;
	int             found_record = 0;	/* record found in history
						 * file */
	char           *fs_token;
	char           *dump_token;
	char            tmpbuffer[256];
	...
	if (access(Dump_hist_file_name, F_OK) == 0) {	/* file exists */
		if (*found_exact_level) {	/* want to scan file for
						 * record */
 ...
 * 
 * Opens or creates the /etc/vdumpdates file.  If the file system exists in the
 * history file, this routine will return the date that corresponds to the
 * most recent backup of that file system in all n-1 levels.  The date it
 * returns is in units of seconds since the epoch time.  If the file system
 * does not exist in the history file or there are no lower dump levels for
 * that file system, the beginning of time is assumed, which does a full
 * backup of all of the files.  If backing up a MSFS file system, the history
 * file is /etc/vdumpdates.
 */
...
get_last_backup_date(char *path, int *found_exact_level)
{
	...
	struct hist_file_rec_t {
		char            file_device[256];	/* dmn and set can be 63
							 * chars long */
		int             file_dump_level;
		char            tm_wday[256];
		char            tm_mon[256];
	...
	if (access(Dump_hist_file_name, F_OK) == 0) {	/* file exists */
		...
		hist_file_fp = fopen(Dump_hist_file_name, "w");
		fclose(hist_file_fp);
		return (0);	/* return beginning of time if source not
				 * found */
	}
	...
	while (1) {
		...
		/*
		 * if fscanf cannot find nine items in the hist file record
		 * then there are no more file systems.  Must drop out of the
		 * while loop. Or, there is an error in /etc/vdumpdates.
		 */
		if (ret <= 0) {
			break;
		...
		if (strcmp(hist_file_rec.file_device, fs_stats.f_mntfromname) == 0) {
			...
			if (hist_file_rec.file_dump_level < Dump_level) {
				...
				/*
				 * To have mktime compute daylight savings
				 * use '-1'
				 */
...
backup_symbolic_link(
		     ...
		     ino_t parent_ino,	/* in - link's directory's inode
					 * number */
		     unsigned long *file_cnt	/* in/out - files backed up
						 * counter */
)
{
	/*
	 * NOTE - the 'names' array below will contain the link's file name
	 * and the link concatenated together (but separated by a null).  The
	...
	link[rcnt] = '\0';	/* readlink() doesn't terminate the link with
				 * a null */
...
backup_hard_link(
		 ...
		 struct stat * link_stat,	/* in - link's file stats */
		 ino_t link_parent_ino	/* in - link's directory's inode
					 * number */
)
{
	/*
	 * NOTE - the 'names' array below will contain the link's file name
	 * and the file name it links to concatenated together (but separated
	 * by a null).  The whole array is null-terminated.  Refer to
	 * 'tapefmt.h'.
	 */
...
)
{
	...
	if (dev_stat->st_nlink > 1) {
		...
		if (link_tbl_add(dev_stat->st_ino, dev_name, parent_ino)) {
			/*
			 * We've already backed up the dev so just backup
			 * this 'dev' as a hard link.
			 */
...
)
{
	...
	if (file_stat->st_nlink > 1) {
		...
		if (link_tbl_add(file_stat->st_ino, file_name, parent_ino)) {
			/*
			 * We've already backed up the file so just backup
			 * this 'file' as a hard link.
			 */
			backup_hard_link(ss, file_name, file_stat, parent_ino);
			return ((long) (0));	/* no Bytes_to_backup for
						 * hardlink */
		}
	}
	if (Verbose) {
	...
	/*
	 * In case of CDFS and DVDFS, stat on small files (<512 bytes),
	 * returns zero blocks. The following 'if' conditional is modified to
	...
	if ((file_stat->st_blocks > 0L) || (file_stat->st_size > 0L)) {
		...
		/*
		 * Get file sparseness.  This currently only works for AdvFS
		 * & UFS.
		 */
		...
		while ((!getmap_failed) &&
			...
			if (sparsemap.extent == NULL) {
				/*
				 * The malloc failed.  Pretend that the
				 * fcntl() failed instead.
				 */
				sparsemap.arraysize = SPARSEMAPSIZE;
				sparsemap.extent = array;
				getmap_failed = TRUE;
		...
		if (getmap_failed) {
			/*
			 * Generate a fake extentmap covering the entire
			 * file.
			 */
			/*
			 * This will fill in pages of zeroes when we aren't
			 * sure
			 */
			/* the file is sparse.                                   */
		...
		for (cur_xtnt = 0; cur_xtnt < sparsemap.numextents; cur_xtnt++) {
			...
			while (!done) {
				...
				if (result > 0) {
					...
					/*
					 * Move data buffer to temp save-set
					 * block
					 */
	...
	/*
	 * Get the current times for the file, but reset using atimes from
	 * the stat buffer. This will keep mtime consistent in case someone
	 * changed the file while we were reading it.
	 */
	if ((Fs_info.fs_type != MOUNT_NFS) &&
	    (Fs_info.fs_type != MOUNT_NFS3)) {
		/*
		 * ignore nfs mounts as we don't want to adjust remote
		 * machine times
		 */
		    ...
		    struct stat * ustatp, struct stat * gstatp)
{
	 ...
	 * Includes standard record headers.  Also, define some shorthand
...
#define fq_r fquota_rec.fquota_fields.fquota
	...
	int             large_limits;	/* TRUE if fileset supports large
					 * quotas */
	...
	for (type = USRQUOTA; type < MAXQUOTAS; type++) {
		...
		case USRQUOTA:
			...
			if (large_limits && !Use_previous_format)
				...
				ugquota32_rec.rec_hdr.rheader.type = RT_UQUOTA32;
			break;
		case GRPQUOTA:
			filename = gfilename;
			statp = gstatp;
			if (large_limits && !Use_previous_format)
				...
				ugquota32_rec.rec_hdr.rheader.type = RT_GQUOTA32;
		...
		if (filename[0] != '\0') {
			...
			if (qfd == ERROR) {
				...
				/*
				 * Use ADVFS extent map routines to skip
				 * sparse holes in quota file
				 */
				...
				if (xtntmap == NULL) {
					...
					/*
					 * Get first allocated byte in quota
					 * file
					 */
				...
				/*
				 * Write a bogus, end-of-file record that
				 * vrestore can recognize and use.
				 */
				if (large_limits && !Use_previous_format) {
					ugq64_r.id = -1;
					mv2blk(ss,
...
{				/* to be marked as an ATTR vs a HDR */
	...
	if (as_attr && (Fs_info.fs_type == MOUNT_MSFS)) {
		...
		if (result == 0) {
			...
			if (result == 0) {
				/*
				 * copy bitfile attributes record to a
				 * save-set block
				 */
				mv2blk(ss,
				       (char *) &bfattr_rec,
...
backup_dir(
	   save_set_t * ss,	/* in - save-set descriptor */
	   char *path_name,	/* in/out - directory's path (must be
				 * MAX_PATH_SZ) */
	   struct stat * dir_stat,	/* in - directory's stats */
	   ino_t dir_ino,	/* in - directory's inode */
	   ino_t parent_ino,	/* in - directory's parent's inode */
	   int mount_dev,	/* in - device root dir is mounted on */
	   time_t last_dump_date/* in - last time files backed up at this
				 * level */
)
{
	void           *dp;
	...
	while ((dir_entry = readdir(dp)) != NULL) {
		...
		case S_IFBLK:
			...
			if ((stat_buf.st_mtime >= last_dump_date) ||
				...
				/*
				 * Add entry to hardlink table only if no. of
				 * links are more than 1, since it is used
				...
				/*
				 * The number of bytes to backup for this
				 * file is generally difficult to determine
				 * given only the stats.  This is especially
				 * true if the file is sparse and has
				 * "chunks" that do not fall on block size
				 * boundaries. Therefore, the number below
				...
				if (add_bytes) {
					...
					/*
					 * If the number of blocks is zero,
					 * add the size of link to
					 * Bytes_to_backup.
					 */

					if (stat_buf.st_blocks == 0) {
						size = stat_buf.st_size;	/* Estimation in case of
										 * symbolic link */
					} else {
	...
	/*
	 * PASS2 - scan the directory and recursively backup each sub
	 * directory
	 */
	dp = opendir(path_name);
	if (dp == NULL) {
	...
	while ((dir_entry = readdir(dp)) != NULL) {
		...
		/*
		 * don't backup AutoFS auto-mounted file systems found along
		 * the way
		 */
...
backup_dir_files(
		 ...
		 ino_t parent_ino,	/* in - directory's parent's inode */
		 int mount_dev,	/* in - device root dir is mounted on */
		 time_t last_dump_date,	/* in - last time files backed up at
					 * this level */
		 unsigned long *file_cnt	/* in/out - files backed up
						 * counter */
)
{
	 ...
	 * check the abort_wanted boolean that could be set by the signal
	 * thread to indicate that we are going to stop prematurely.
	 */
	...
	while ((dir_entry = readdir(dp)) != NULL) {
		...
		/*
		 * since nfs mounts have inodes from the remote nfs server's
		 * mount point, and that mount point could be a file system
		 * of type ufs, and ufs allows inodes of 4 or 5, it would
		 * result in not backing up the nfs directory that happens to
		 * have an inode of 4 or 5.  To protect from this condition,
		 * test that the quota files we are trying to skip here are
		 * of regular type (S_IFREG) and not a IFDIR like an nfs
		 * mounted directory is.
		 */
		if ((Fs_info.fs_type == MOUNT_MSFS) &&	/* fs being dumped is
							 * advfs */
		    ((stat_buf.st_mode & S_IFMT) == S_IFREG) &&	/* file is a regular
								 * file type, */
		/* meaning not an nfs mount with inode==4,5 */
		    ((stat_buf.st_ino == USR_QUOTA_INO) ||
		     (stat_buf.st_ino == GRP_QUOTA_INO))) {	/* file is a advfs quota
								 * file */
			/* skip quota files */
			continue;
		}
		...
		if ((stat_buf.st_mtime >= last_dump_date) ||
			...
			case S_IFDIR:
				/*
				 * avoid creating entries for AutoFS
				 * intercept points
				 */
...
{
	struct stat     stat_buf, uqstat_buf, gqstat_buf;
	char            dir_name[MAX_PATH_SZ + FILE_NAME_SIZE + 1],	/* DON'T MAKE SMALLER */
	                uqfile_name[MAX_PATH_SZ + FILE_NAME_SIZE + 1], gqfile_name[MAX_PATH_SZ + FILE_NAME_SIZE + 1];
	...
	/*
	 * Since the fileset will be consistant with its inodes and the hard
	 * links, we lookup for missed hard links only when backing-up
	 * unmounted directories using '-D' option.
	 */
	if (Backup_subtree)
	...
	/*
	 * If this is a backup of a full AdvFS fileset, back up the quota
	 * files first.
	 */
	if (!Backup_subtree && Fs_info.fs_type == MOUNT_MSFS) {
		if (user_is_root) {
...

{
	...
	while (not_valid_write) {
		if (rcnt = write(fd, blk, cnt) == ERROR) {
			if (errno == ENOSPC) {	/* [28] No space left on
						 * device */
...
make_norewind_dev(char *devname)
{
	...
	if ((p_c1 = strstr(temp_name, "/dev")) == NULL) {
		/*
		 * doesn't have either the OSF style tape drive name or the
		 * new style device name, so warn user and do nothing
		 */
		fprintf(stderr,
...

{
	...
	_fake_enospc:
			...
			/*
			 * Commented the following isatty() call to support
			 * vdump running as cron/batch jobs.
			 */
#if 0
			if (!isatty(0)) {
				fprintf(stderr,
			...
			if ((strncmp(os_name, "ULTRIX", 6) == 0) && (Dev_type == TAPE)) {
				ultrix_rmtdevice_stat(Device_name);	/* we can check if
									 * ultrix on other end */
			}
#else
			/*
			...
			/*
			 * Commented the following isatty() and op_retry()
			 * call to support vdump running as cron/batch jobs.
			 */
#if 0
			if (!isatty(0)) {
...
list_fs_to_backup()
{
	FILE           *fstab_file_fp, *dump_file_fp;
	/* int ret;  */
	pid_t           pid;
	int            *waitstatus;
	char           *arglist[2];
	long            seek_count = 1;	/* seek in multiples of long words */
	int             first_time = 1,	/* first time to look at vdumpdates
					 * file */
	                first_hour = 1,	/* first time to look at t.tm_hour */
	                not_found = 1;	/* device file found in vdumpdates
					 * file */
	time_t          todays_date;	/* today's date in seconds */
	struct tm       t;
	int             file_dump_level, tm_year;
	char            fstab_device[52], fstab_path[52], fstab_type[12], fstab_rest[LINE_MAX], dump_device[17], tm_wday[4], tm_mon[4];
	...
	while (!feof(fstab_file_fp)) {	/* for each file system in fstab */
		...
		if ((strcmp(fstab_type, "msfs") == 0) ||
			...
			/*
	                 * if not nfs mounted
	                 */
			not_found = 1;	/* device not found in
					 * /etc/vdumpdates */
			...
			while (not_found) {
				...
				if (fseek(dump_file_fp, seek_count, SEEK_END) == 0) {
					...
					if (first_time) {
						...
						/*
						 * if devices match then
						 * compare device's date to
						 * today's date (must be less
						 * than 86400=1day seconds)
						 */
						...
						if (strcmp(dump_device, fstab_device) == 0) {
							...
							not_found = 0;	/* found the exact file
									 * system */
						}	/* end if for strncmp */
					}	/* end if for first_time */
					seek_count = seek_count - 1;	/* search for next
									 * record */

				} else {	/* end of vdumpdates file */
					not_found = 0;	/* didn't find file
							 * system so list it */
					fprintf(stderr,
...
typedef struct link_entry {
	...
	ino_t           parent_ino;	/* the file's directory's inode
					 * number */
...
link_tbl_lookup(
		...
		ino_t * parent_ino	/* out - file's directory's inode
					 * number */
)
{
	int             idx = -1;
	...
	while (cur_ent != NULL) {
		...
		if (cur_ent->ino == ino) {
			...
			*name = cur_ent->name;	/* don't strcpy since entries
						 * never die */
...
typedef struct hardlink_tbl_entry {
	ino_t           ino;	/* the file's inode number */
	uint_t          nlink_count;	/* st_nlink count for a hard linked
					 * file. */
	uint_t          links_found;	/* No. of hard links found for this
					 * file. */
...
)
{
	...
	while (cur_ent != NULL) {
		if (cur_ent->ino == ino) {
			 ...
			 * found for a file should be less than OR equal to
			 * the no. of st_nlinks.
			 */
	...
	new_ent->links_found = 1;	/* because current inode is one of
					 * the links. */
 ...
 * free_hardlink_tbl
 * 
 * This function frees the memory allocated by hardlink_tbl_add function.
 */
void 
free_hardlink_tbl(void)
{
	int             idx = 0;
 ...
 * lookup_for_missed_hardlinks
 * 
 * This function checks for the probable mismatch in the link count for a file
 * and the actual hardlinks found during the estimation navigation. This
 * mismatch may be due to hard links beyond dump directory. The function is
 * called when all the dirs are navigated in estimation stage.
 */

void 
lookup_for_missed_hardlinks(void)
{
	int             idx = 0, status = 0;
	...
	/*
	 * Since we have already cd'ed into the source dir we can statfs on
	 * the pwd and know the mount point. We need to print the message
	 * with mount point case hard links are missed.
	 */
	...
	for (idx = 0; idx < LINK_TBL_SZ; idx++) {
		...
		while (lookup_tbl != NULL) {
			/*
			 * Found a valid Hash table entry. Now chk for link
			 * count
			 */
...
prepare_signal_handlers(void)
{
	...
	/*
	 * Spawn a new thread to handle signals; new thread runs
	 * signal_thread()
	 */
	thread__attr_create(&thread_attr);
...
signal_thread(void *arg)
{
	...
	/*
	 * This thread is responsible for catching any asynchronous signals
	 * that are delivered to this process and handling them in an
	 * appropriate fashion. Signals currently handled are:  SIGINT,
	 * SIGQUIT, SIGTERM
	 */
	sigemptyset(newsetp);
	...
	while (!abort) {
		...
		if (sigwait(newsetp, &sig_delivered))
			...
			/*
			 * non-interactive or unblocked signal received;
			 * terminate
			 */
			abort = 1;
		}
...
/*
 * Interactive query to abort; return TRUE if abort desired, else FALSE. If
 * stdout is being used for output, this will return TRUE.  This routine may
 * be called from threads other than the signal-thread.
 */
int 
want_abort()
{
...
/*
 * This thread handles the I/O for the main thread.  The 'writes_are_ok'
 * variable is normally TRUE;  it is turned off when: a. there is an error on
 * the output and the user signified that they wanted to abort, or b. the
 * signal_thread signifies an abort is pending. This variable is manipulated
 * only in this routine, and is therefore not protected by a mutex. This
 * turning off of actual I/O allows this thread  to continue to run until the
 * main thread signals it to terminate.  When processing large files, the
 * main thread may require the write thread to recycle some buffers before it
 * can detect that an abort is desired, and keeping this thread running
	...
	while (TRUE) {
		...
		if (msg.type == MSG_WRITE_BLK) {
			...
			if (writes_are_ok) {
				...
				/*
				 * On output error, if user wants abort, do
				 * no more output
				 */
				if (retval == ERROR) {
					mutex__lock(abrt_mutex);
 ...
 */
static int 
check_if_mountpoint(const char *pathname, const char *mount_point)
{
