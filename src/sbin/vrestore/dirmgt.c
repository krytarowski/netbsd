 ...
 * Abstract:
 * 
 * Defines routines to manage and access a logical directory (such as the one
 * used by the restore program).
 * 
** THESE ROUTINES ARE NOT CURRENTLY THREAD SAFE **
 * 
 * A logical directory is a UNIX file that contains the contents of all the
 * directories that have been restored from the directory section of a
 * save-set.  The directories are restored from the save-set to the logical
 * directory (also called the 'directory file') one at a time and are place
 * one after another in the dir file:
 * 
 * dir1 dir2 dir3 .... dirN
 * 
 * As each directory is placed into the dir file we also create an entry for it
 * in the Inode Hash Table.  The table entry describe the directory's
 * position within the dir file (we use the byte offset into the dir file so
 * that we can use lseek() to locate the start of a directory) and the length
 * of the directory (refer to inotbl.c for more info).
 * 
 * Date:
 * 
...
char            Cwd_name[MAX_PATH_SZ];	/* current working dir's full path
					 * name */
...
)
{
	 ...
	 * deleting the dir file when it is closed when we're done with it.
	 */
	unlink(Tmp_dname);
...
)
{
	...
	/*
         ...
         */
	/*
	 * Pass the dir_stat instead of dir_inode. we need to preserve times,
	 * modes and permissions also.
	 */
...
)
{
	...
	dir_desc->fd = dir_fd;
	dir_desc->buf_offset = DIRBLKSIZ;	/* this makes the buf appear
						 * invalid */
...
)
{
	...
	while (TRUE) {
		...
		if (dp->d_reclen == 0) {
			...
			/*
			 * reached zero-filled end of this block; goto next
			 * block
			 */
			dir_desc->buf_offset = DIRBLKSIZ;
			continue;
...
char           *
dir_get_path(
	     ino_t ino,		/* in - inode for which to generate a path
				 * name */
...
)
{
	...
	if ((ent != NULL) && (ent->type == DIR_ENT)) {
		...
		/*
		 * adding a file don't fail to add entries on hard link
		 * siblings if there is an entry with a matching inode but
		 * not a perfect match, then it too must be entered else its
		 * path will not be there when it is time to make the hard
		 * link
		 */
