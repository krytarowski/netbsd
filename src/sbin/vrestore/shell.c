...
static void 
sort_and_print_files(int name_cnt, int max_name_sz,
		     file_desc_t files[]);
...
ls(int dir_fd, char *path, int list_all)
{
	...
	if (status == OKAY) {
		...
		/*
		 * 'path' is not a directory.  See if it is a file we know
		 * about
		 */
	...
	while ((dir_entry = dir_read(&dd)) != NULL) {
		...
		if (list_all || ino_tbl_restore_me(ent, dirs_ent)) {
			/*
			 * Allocate space for the file name.  Note that this
			 * routine doesn't deallocate the space.  This is
			 * done by sort_and_print_files().
			 */
