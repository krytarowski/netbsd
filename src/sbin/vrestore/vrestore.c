...
typedef enum {
	R_UNKNOWN, R_CRC_AND_XOR, R_NONE
}               reliability_t;
...
restore_dir_data(
		 int dir_fd,	/* in - logical directory handle */
		 int record_size,	/* in - Number of bytes in the data
					 * record */
		 struct blk_t * blk,	/* in - The block containing the data
					 * record */
...
restore_file_data(
		  int fd,
		  unsigned int record_size,	/* record_size can exceed
						 * 'short' */
...
void
create_device_file(
		   struct dev_attr_t * dev_attr,	/* in - dev attributes
							 * record */
		   char *name,	/* in - dev name */
		   unsigned long *file_cnt,	/* in/out - Number of files
						 * restored counter */
...
set_file_dir_attr(
		  struct dattr_t * dir_attr,	/* in - dir attributes record */
		  mlBfAttributesT * bfAttr,	/* in - dir's bitfile
						 * attributes */
		  mlBfAttributesT * bfIAttr,	/* in - dir's inheritable
						 * attributes */
...
static int
                File_spec_cnt = 0, Tape_number = 1, File_spec = FALSE, Verbose = FALSE, Show = FALSE,	/* If true, user just
													 * wants to see what's
													 * in the saveset */
                List = FALSE;

static int      Abort_restore = 0;	/* If 1 then user requested to abort */
static int      AlreadyPrintedNoQuotas = 0;	/* If 1 then already printed
						 * this message */
...
static char
               *File_specs[MAX_FILE_SPECS], *Source = NULL, *Target = NULL;
...
int             prop_name_from_file = 0;	/* flag whether setting prop
						 * list on files name or
						 * directories name */
int             Debug = 0;
int             Src_fd = -1;
...
char           *open_file = NULL;	/* regular file that is currently
					 * open */
...
{
	/* argv variables */
	int             g = 0, c, l = 0, i = 0, t = 0, s = 0, r = 0, D = 0,
	                q = 0, Q = 0, v = 0, m = 0, w = 0, o = 0, f = 0,
	                x = 0, h = 0, n = 0, e = 0, I = 0;
	extern int      optind;
	extern char    *optarg;
	char           *p;
	char           *overwrite, *f_file;
	char           *remote_user_name = NULL;
	char           *remote_host_name = NULL;
	char           *Device_name_ptr = NULL;
	char           *tmp_ptr = NULL;
	char           *ptr = NULL;
	int             j, done = FALSE, device = FALSE, help = FALSE, show_stats = FALSE, error = FALSE, compressed_data = FALSE, xor_blks = 0;
	 ...
	 * create a mutex to guard the 'close_device' variable.
	 */
	...
	if (posix_style(argc, argv)) {	/* if POSIX-style of args */
		...
		while ((c = getopt(argc, argv, "gD:liqQtsrvVmneo:f:xhI")) != EOF) {
			...
			case 't':
				...
				t++;
				/*
				 * user just wants to see what's in the
				 * saveset
				 */
				Show = TRUE;
				break;
	...
	/*
	 * Do not restore ownership, permissions, ACLs on top level directory
	 * if selective restoration is being done.
	 */
	...
	*Device_name_ptr = '\0';
	...
	if (!device) {
		...
		if ((Device_name_ptr = getenv("TAPE")) == NULL) {
			Source = DEFTAPE;	/* "/dev/tape/tape0_d1" in
						 * mtio.h */
		} else {
...
get_extraction_list(int dir_fd)
{
	...
	if (File_spec_cnt == 0) {
		/*
		 * -x was specified but no files were given; restore all
		 * files
		 */
...
)
{
	...
	case RS_FILE_DATA:
		 ...
		 * RS_BF_FILE_ATTR but under ufs, it could follow itself or
		 * either RS_FILE_ATTR or RS_PROPLIST
		 */
...
restore(
	...
	reliability_t reliability,	/* in - The save-set's reliabilty
					 * mode */
...
{
	enum {
		RESTORING_FILES, RESTORING_DIRS
	}               mode;

	int
	                restore_fd = CLOSED,	/* file desc. of file being
						 * restored        */
	                uq_fd = CLOSED,	/* file desc. of user quota file            */
	                gq_fd = CLOSED,	/* file desc. of group quota file           */
	                dp = CLOSED, dir_restored = FALSE, block_num = 1, expected_vol_num = 1, expected_blk_num = 1, restore_uquotas = FALSE,	/* TRUE if user quotas
																		 * are to be restored   */
	                restore_gquotas = FALSE,	/* TRUE if group quotas
							 * are to be restored  */
	                qwarned_already = FALSE,	/* TRUE if user has been
							 * warned re. quotas  */
	                user_is_root = FALSE,	/* TRUE if user is root (uid
						 * = 0)           */
	                target_q_on = TRUE,	/* TRUE if target fs has
						 * enabled quotas     */
	                ...
	                ret_val, err, status = OKAY;	/* status returned by
							 * rblk_get              */
	...
	char           *opt, *cp = NULL;	/* For parsing /etc/fstab
						 * entries   */
	...
	while (!eovs && (0 == Abort_restore)) {
		...
		if (status == ERR_BAD_BLK) {
			...
			rblk_free(rblkh, blk_id);	/* release the current
							 * block */
				...
				if (List) {
					...
					if (Quota_data_present) {
						int             type,	/* Loop counter              */
						                qinode,	/* Quota file inode
									 * number   */
						               *restore_switch,	/* TRUE if restoring
										 * quotas  */
						               *qfd;	/* Open file descriptor      */
						char           *real_qfile_name,	/* Real quota file name      */
						               *fstab_keyword,	/* /etc/fstab quota
										 * keyword  */
						               *prototype_qfile_name;	/* Prototypical quota
											 * file   */
		...
		while ((data_idx < MAX_DATA_SIZE(Blk_size)) && !eob && !eovs) {
			 ...
			 * old_record_header_t or new record_header_t with
			 * the help Version in Block header. If it is old
			 * record, then copy the data into a new format.
			 */
			...
			/*
			 * Based on the mode and record type perform the
			 * appropriate record-specific actions.
			 */
			...
			case RESTORING_DIRS:
				...
				/*
				 * * Handle the record types that are valid
				 * while * restoring dirs.
				 */

				switch (rhdr.rheader.type) {

				case RT_DIR_HDR:	/* directory header
							 * record */
				...
				case RT_DIR_DATA:	/* directory data record */
					...
					break;
#ifdef XTENDED_ATTRBS
				case RT_PROPLIST:	/* extended attributes
							 * record */
				...
				case RT_FILLER:	/* filler record          */
					...
					if (List)
						list_rec_hdr(catgets(catd, S_VRESTORE1, VRESTORE22, "filler"), &rhdr.rheader,
							     stdout);
					eob = TRUE;
					break;
			...
			case RESTORING_FILES:
				...
				case RT_DIR_ATTR:	/* directory attributes
							 * record */
					...
					if (!First_dir_seen && top_level_dir) {	/* First directory is
										 * special */
				...
				case RT_BF_DIR_ATTR:	/* bitfile dir
							 * attributes record */
					state = STATE_TRANSITION(state, RS_BF_DIR_ATTR);

					/*
					 * List the Bitfile attributes for -l
					 * option
					 */
					if (List) {
						bcopy(&blk->data[data_idx],
				...
				case RT_HARD_LINK:	/* hard link attributes
							 * rec */
				...
				case RT_SYM_LINK:	/* sym link attributes
							 * rec */
				...
				case RT_FILE_ATTR:	/* file attributes
							 * record */
					...
					ret_val = create_file(&fattr.fattr,
							    ...
							    compressed_data,
							      lzwh);
					/*
					 * create_file returns OKAY when List
					 * is performed AND when a file is
					 * indeed created or can be
					 * overwritten If a file already
				...
				case RT_BF_FILE_ATTR:	/* bitfile attributes
							 * record */
					...
					&& (Fs_info.fs_type == MOUNT_MSFS)) {
						bcopy(&blk->data[data_idx],
						      ...
						      sizeof(struct bf_attr_rec_t));
						/*
						 * Restore the file's bitfile
						 * attributes if restoring to
						 ...
						 * seg_decrCnt > 0 to
						 * bf_attr.bfAttr.attr.stripe.
						 * segmentCnt > 1 and also
						 * changed the variable in
						 * the fprintf from
						 * seg_decrCnt to
						 * bf_attr.bfAttr.attr.stripe.
						 * segmentCnt to display the
						 * correct number of volumes
						 * on which the file is
						 * striped while restoring.
						 */
				...
				case RT_FQUOTA:	/* Fileset quota record */
					...
					if ((Show) || (!List && Fs_info.fs_type != MOUNT_MSFS));	/* Do nothing; see note
													 * above. */
	...
	if (eovs) {
		...
		if (Dev_type == TAPE && Rewind) {
			struct mtop     t_op = {MTFSF, 1};	/* MTFSF forward to next
								 * file */
	...
	/*
	 * Now, that the dir. structure and data is restored, restore dir.
	 * attrs.
	 */
...
)
{
	...
	if (bhdr->flags == 0) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE78, "none"));
	}
	if (bhdr->flags & BF_COMPRESSED) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE79, "<compressed>"));
	}
	if (bhdr->flags & BF_XOR_BLOCK) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE80, "<XOR block>"));
	}
	if (bhdr->flags & BF_FILES) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE81, "<files>"));
	}
	if (bhdr->flags & BF_DIRECTORIES) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE82, "<dirs>"));
	}
	if (bhdr->flags & BF_QUOTA_DATA) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE176, "<quotas>"));
	}
	if (bhdr->flags & BF_USER_QUOTA_DATA) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE225, "<user quotas>"));
	}
	if (bhdr->flags & BF_GROUP_QUOTA_DATA) {
		fprintf(std, catgets(catd, S_VRESTORE1, VRESTORE226, "<group quotas>"));
	}
	fprintf(std, "\n");
...
void
list_hard_link_attr(
		    struct hlink_t * hlink,	/* in - ptr to hard link
						 * attributes */
		    char *file_name,	/* in - file name */
...
void
list_bf_file_attr(
		  struct bf_attr_rec_t bf_attr	/* in - ptr to sym link
						 * attributes */
)
{
	char           *iotype;
...
void
list_sym_link_attr(
		   struct slink_t * slink,	/* in - ptr to sym link
						 * attributes */
		   char *link_name,	/* in - link name */
		   char *link	/* in - link */
)
...
)
{
	...
	int             nchunks_to_read = 2;	/* Enough to read new summary
						 * rec */
	...
	if (Dev_type == TAPE && fixed_blksize == 0) {
		/*
		 * Modified rsize calculation to use OLD_MAX_CHUNKS as in a
		 * clustered setup drd is unable to handle MAX_CHUNKS large a
		 * read OR write.
		 */
	...
	/*
	 * Must compare the version of vrestore to the vdump version of the
	 * tape to ensure upward compatibility only.  Version is used later
	 * to for backward compatibility (struct stat).
	 */
	...
	} else {		/* Not a summary rec - something seriously
				 * wrong */
		fprintf(stderr,
			catgets(catd, S_VRESTORE1, VRESTORE115,
		 "%s: unable to use save-set; invalid or corrupt format\n"),
...
check_overwrite(char *file_name)
{
	...
	int             not_valid = 1;	/* assume overwrite answer isn't
					 * valid */
	int             fset_quotas = FALSE;	/* TRUE if file_name is
						 * fileset quotas */
...
int
close_file(
	   struct fattr_t * fattr,	/* in - File attributes of file to be
					 * restored */
	   int *fd,		/* in - in/out file descriptor */
	   char *file_name,	/* in - name of restored file */
	   int compressed,	/* in - flag if compressed */
...
)
{
	...
	if (fstat(*fd, &newFstat) < 0) {
		/*
		 * fstat failed, so initialize uid & gid with invalid value
		 * (-1)
		 */
		newFstat.st_uid = -1;
		newFstat.st_gid = -1;
...
)
{
	...
	if (compressed_data) {
		/*
		 * Uncompress the data in the data record and write it to the
		 * logical directory.  This has to be done in a loop since it
		 * is impossible to determine the buffer size needed for the
		 * uncompressed data.  So the uncompression is done
		 * incrementally.
		 */
...
)
{
	...
	if (Show) {
		...
		if (top_level_dir)
			if (dir_attr->dir_stat.st_ino == dir_attr->parent_ino) {
				/*
				 * don't need to restore root as this is
				 * really our 'Target'
				 */
				return 0;
			}
	...
	/*
	 * Since setproplist() syscall does not set property list for top
	 * level directory ".", it is necessary to pass the complete path
	 * name of Target directory (where files are restored). This is done
	 * by getcwd() call.
	 */
	if (!top_level_dir) {
		if (strcmp(name, ".") == 0) {
...
create_file(
	    struct fattr_t * fattr,	/* in - File attributes of file to be
					 * restored */
	    int *fd,		/* out - File descriptor of file to be
				 * restored */
	    char *name,		/* in - Name of file to be restored */
	    char *file_name,	/* out - file name and its path pre-appended */
	    unsigned long *file_cnt,	/* in/out - Number of files restored
					 * counter */
...
)
{
	...
	if (Show) {
		...
		/*
		 * Ensure that file is created in the correct filesystem.
		 * Like, use the path part of 'file_name'. This will ensure
		 * that the rename in close_file() works. May need to add
		 * call to mkpath()?!
		 */
...
restore_file_data(
		  ...
		  unsigned long d_offset,	/* in - offset into file for
						 * rec */
		  unsigned long *tot_bytes_restored,	/* in/out - total bytes
							 * or all recs */
...
{
	int             retry, wcnt, total_bytes_decoded, bytes_decoded,
	                bytes_xferred;
	...
	if (compressed_data) {
		/*
		 * Uncompress the data in the file data record and write it
		 * to the file being restore.  This has to be done in a loop
		 * since it is impossible to determine the buffer size needed
		 * for the uncompressed data.  So the uncompression is done
		 * incrementally.
		 */
		total_bytes_decoded = 0;
...
{
	int             is_eof_record = FALSE,	/* TRUE if cur. rec is EOF
						 * rec */
	...
	static int      first_user_rec = TRUE,	/* TRUE if this is first user
						 * rec */
	                first_group_rec = TRUE,	/* TRUE if this is first
						 * group rec */
	                last_id_restored = -1;	/* ID of last quota rec
						 * restored */
	...
	int             large_source_limits,	/* TRUE if source fileset
						 * supports large quotas */
	                large_target_limits;	/* TRUE if target fileset
						 * supports large quotas */
	...
	if (type == RT_FQUOTA) {
		...
		if (!is_eof_record) {
			...
			if (large_target_limits) {
				...
				if (large_source_limits) {	/* large source, large
								 * target */
				...
				if (large_source_limits) {	/* large source, small
								 * target */
					last_id_restored = ug64_recp->id;
...
)
{
	...
	if (advfs_xtntmap_next_byte(xtntmap, curbyte, &nextbyte) >= 0) {
		...
		while (nextbyte < (long) (lastId) * dqsize) {
			/*
			 * Assume that most quotas are non-zero.  So instead
			 * of reading the quota file to see if this quota is
			 * already zero, call quotactl to set it to zero.
			 */
		...
		if ((rcnt = read(Src_fd, blk, cnt)) > 0) {
			...
			if (Src_fd >= 0) {
				...
				if (close_device != NULL) {
					/*
					 * since close_device is a function
					 * pointer we don't want to call it
			...
			if (strncmp(os_name, "ULTRIX", 6) == 0) {
				ultrix_rmtdevice_stat(Source);	/* we can check if
								 * ultrix on other end */
			}
			/*
			 * if not ultrix, then not much we can do because of
			 * rmt limitations
			 */
#else
			/*
...
tape_reset_save_set(int fd)
{
	 ...
	 */
	struct mtop     t_op = {MTREW, 1};	/* MTREW rewinds to begin of
						 * tape */
 ...
 * stdin is being used for input, or the user cannot respond, then this
...
abort_now()
{
	...
	if (close_device != NULL) {
		/*
		 * since close_device is a function pointer we don't want to
		 * call it if it hasn't been initialized yet.
		 */
		close_device(Src_fd);	/* close the save-set */
	}
	mutex__unlock(dev_mutex);
...
)
{
	...
	if (rhdr.rheader.size == 0) {
		/*
		 * avoid lots of needless code on empty records and
		 * malloc'ing zero bytes (that would look like a malloc
		 * failure)
		 */
	...
	if (compressed) {
		...
		while (total_bytes_decoded < rhdr.rheader.size) {
			...
			if (err == -1) {
				/*
				 * when uncompress doesn't like the data its
				 * useless to continue with this record
				 */
			...
			total_bytes_decoded += bytes_decoded;
			/*
			 * we've transferred some translated data to the
			 * buffer
			 */
	...
	}			/* proplist data is now ready to be used to
				 * set proplists */
	...
	/*
	 * if we are holding proplist data from a previous call combine it
	...
	/*
	 * move the current proplist data into the buffer (possibly combining
	 * with held data) Note: the data is in different buffers depending
	...
	if (List) {
		...
		if (rhdr.rheader.flags != RF_SPAN_RECORD) {
			...
			/*
			 * get and print out the name, length and value for
			 * each entry
			 */
			while (buf_size > 0) {
				int             tmp_size;
		...
		if (rhdr.rheader.flags != RF_SPAN_RECORD) {
			...
			if (fd != CLOSED) {	/* Regular file */
				...
				if ((Check_this_dir) && (last_name[0] == 0) && (top_level_dir)) {
					/*
					 * Top level directory name is blank
					 * (vdump doesn't save the name). We
					 * won't restore it. Therefore we
					 * can't try to restore it's
					 * proplist. This code will not break
					 * if we start passing a true
					 * directory name
					 */
					Check_this_dir = 0;	/* avoid checking again */
				} else {
					/*
					 * not top level directory, so
					 * process it
					 */
					if (Preserve_modes) {
						/*
						 * wait till all the
						 * dirs/files are restored to
						 * restore the proplist on
						 * the dir. This may get
						 * inherited otherwise.
						 */
						/*
						 * Find the corresponding
						 * ino_tbl entry and assign
						 * the property list buffer
						...
						/*
						 * dir_ino is same as
						 * Cwd_ino. dir_lookup() and
						 * dir_find() cannot handle
						 * top level dir.
						 */
...
char           *
walk_prop_list(int *setnbytes, int num_ext_attr_bytes,
	       char *property_list_input)
{
