 ...
 * 
 * The Inode Hash Table (Ino_tbl) is used to keep track of the files and
 * directories to be restored.  It is a hash table that uses inode numbers
 * has the hash key.  Each hash bucket points to a list of table entries
 * whose inode numbers hash to the same bucket.
 * 
 * All directories on the save-set have an entry in the Ino_tbl. A file will
 * have an entry only if the user explicitly specified that the file should
 * or should not be restored. Files that are selected for restoration because
 * their parent directory was specified do not necessarily have an entry in
 * the table.  This is because, unless specified otherwise, all descendents
 * of a directory are restored automatically when a directory is selected for
 * restoration.  Therefore, there is no need to add an entry for every file
 * into the table. Examples of when a file would have an entry in the table
 * are:
 * 
 * 1.  The user explicitly specified that the file should be restored regardless
 * of whether or not its parent directory was specified.
 * 
 * 2.  The user has specified that a directory is to be restored but has also
 * specified that a file in the directory is not to be specified.
 * 
 * So, the Ino_tbl is used to identify all directories and all file's that were
 * explicity selected (to be restored or not restored).
 * 
 * The Ino_tbl also has the properties of a 'tree'.  Each entry points to its
 * parent directory's entry.  Each entry is also an element of a siblings
 * list which identifies all files and directories in the same directory.
 * The siblings list is pointed to by it's parent directory via it's
 * 'children' pointer (in other words, each directory entry has a 'children'
 * pointer that points to the head of a list of its children).
 * 
 * Date:
 * 
...
)
{
	...
	*new_ent = Nil_dir_entry;
	...
	if (Preserve_modes) {
		...
		/*
		 * Before setting-up the data structure for the dir. chk. is
		 * the user is having permissions to restore them on the
		 * restored dir.
		 */
	...
	if (dir_stat->st_ino == parent_ino) {
		...
		name = get_name(dir_name);	/* strip off any preceding
						 * path name */
	}
...
ino_tbl_add_file(
		 ...
		 char *file_name,	/* in - file's name (can't be full
					 * path name) */
...
)
{
	if (my_ent == NULL) {
		/*
		 * Must be a file that we don't have in the table.
		 * Therefore, it should be restored only if it's parent is to
		 * be restored.
		 */
...
restore_dir_attr(void)
{
	...
	while (last_order) {
		/*
		 * Now start restoring the dir. attrs from the last restored
		 * dir.
		 */
		curr_ent = last_order->ino_ptr;

		/*
		 * restore_me or restore_cnt for some dirs could be TRUE
		 * during selective restoration.
		 */
		if ((curr_ent != NULL) && (curr_ent->restore_me || curr_ent->restore_cnt)) {
			...
			/*
			 * Chk. if restore_path is a dir. only. In a rare
			 * case this could be a file also.
			 */
			if ((stat(restore_path, &dir_stat) == 0)
			    && (!S_ISDIR(dir_stat.st_mode))) {
				/*
				 * This is not the dir. we intended to
				 * restore.
				 */
			...
			/*
			 * Do not restore dir attrs on top level dir when
			 * selective (-x) or when interactive (-i) restore.
			 */
			if ((strcmp(curr_ent->name, "/") == 0) && (top_level_dir)) {
				/*
				 * Do nothing here if the above conditional
				 * is true.
				 */
			}
			/* Restore attributes only if owner */
				...
				if (chown(restore_path, fuid, fgid) < 0) {
					...
					/*
					 * Restore saved ACLs/Extended
					 * attributes now. There is still a
					 * chance that property list/ACLs are
					 * existing on this dir. If there are
					 * more sets of property lists on the
					 * existing dir., setproplist()
					 * overwrites only the overlapping
					 * sets. In order to make sure the
					 * restored dir. will only have
					 * property list in the archived
					 * buffer, delete the existing
					 * property list and then
					 * setproplist.
					 */
