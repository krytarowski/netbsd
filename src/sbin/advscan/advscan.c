...
void 
process(fd, cnt, lp)
	int fd;			/* File descriptor of partition to be
				 * processed */
	int cnt;		/* Which partition (0 == a, 1 == b, ... , 7 ==
				 * h) */
	struct disklabel *lp;	/* Duh. */
{
	...
	if (lp == NULL) {
		...
		part->correspond_flag = 0;	/* 0 means found partition on
						 * disk */
		...
		if (print_geom) {
			...
			if (boot)
				printf(catgets(catd, 1, 3, ", bootable"));
			printf("\n");
	...
	if (blkcnt == parts->p_size) {
		...
		if (!print_geom && (state_flag != Rmvoled)) {
			...
			/* Check to see if this partition exactly (size &
			 * start block) overlaps another partition already in
			 * this domain's table. */
			...
			for (i = 0; i < domain_table[domain_slot].number_parts; i++) {
				...
				if (!strncmp(part_path, orig_part_path, strlen(part_path) - 1)) {
					/* These volumes are on the same
					 * device. See if they start at the
					 * same block. */
			...
			if (!strncmp(devname, "/dev/rr", 7)) {	/* unraw for old dev
								 * names */
	...
	char *argv[];
{
	...
	for (j = 0; j < disks; j++) {
		...
		if (FD == NULL) {
			...
			strcat(rawdevname, "a");	/* Append a partition
							 * name */
			fsl_find_raw_name(rawdevname, p, FS_DIR_MAX_NAME);
			p[strlen(p) - 1] = '\0';	/* Remove the partition
							 * name */
		} else {
			/* This is a diskgroup */
	...
	if (!quiet && !fix) {
		...
		for (i = 0; i < domain_cnt; i++) {
			...
			for (j = 0; j < domain_table[i].number_parts; j++) {
				...
				if (part->wrong_dir != 1) {	/* Part in correct
								 * directory. */
	...
	if (fix) {
		for (i = 0; i < domain_cnt; i++) {
			...
			if (domain_table[i].num_links == domain_table[i].disk_count) {
				/* more parts than links and count. Should be
				 * OK. */
				...
				/* less parts than links and domain count -
				 * can't fix */
				if (domain_table[i].number_parts < domain_table[i].disk_count) {
...
)
{
	...
	while ((devs = readdir(FD)) != NULL) {
		...
		while ((devss = readdir(FDD)) != NULL) {
			...
			if (flag == 4) {
				got_it = FALSE;
				/* scan the domain table (all partitions in
				 * all domains) for this partition */
				if (got_domain == FALSE) {
					for (i = 0; i < domain_cnt; i++) {
						for (j = 0; j < domain_table[i].number_parts; j++) {
							part = domain_table[i].parts[j];
					...
					/* already in the domain, just look
					 * for the partition */
				} else {
			...
			if (flag == 5) {
				...
				if (islsm64(raw, &lsmSize)) {
					...
					/* Now we know it is NOT, so convert
					 * to partition a */
