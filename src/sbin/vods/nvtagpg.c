...
process_all(bitFileT * bfp, char *argv[], int argc, int optind, int flags)
{
	...
	/*
	 * the number of valid pages may be less than the number of pages in
	 * the extent map. tag directories are allocated in units of 8 pages,
	 * but not all of them are necessarily valid.
	 */
	if (bfp->pages == 0)
		exit(0);
...
print_tagpage(bitFileT * bfp, bsTDirPgT * pdata, lbnT lbn, int flags)
{
	...
	/*
	 * FIX. if currPage != bfp->pgNum then don't look at tags unless
	 * verbose
	 */
	for (slot = 0; slot < BS_TD_TAGS_PG; slot++) {
...
print_tag_entry(bitFileT * bfp, bsTDirPgT * pdata, int slot, int flg)
{
	...
	if (bfp->pgNum != (unsigned) -1) {
		...
		/*
		 * We trust the file page number more than the data at the
		 * page.
		 */
		pgNum = bfp->pgNum;
		...
		/*
		 * We have only the data on the page (currPage) to tell the
		 * page num.
		 */
		...
		if (tagNum < 0) {
			/*
			 * Use this as a flag that we can't find a reasonable
			 * tag number.
			 */
			tagNum = -1;
		}
...
test_all_tags(bitFileT * rootTagBfP, uint ** dmnMeta)
{
	...
	for (pg = 0; pg < rootPgs; pg++) {
		...
		for (entry = 0; entry < BS_TD_TAGS_PG; entry++) {
			...
			/*
			 * test_mcell nneds the set tag from tagBfP to test
			 * the mcells.
			 */
