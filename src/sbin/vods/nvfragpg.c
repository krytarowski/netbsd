...
void 
print_verbose_free_order_frags(bitFileT *, bsBfSetAttrT *, fragT *, int **,
			       int *, int);
	...
	char           *argv[];
{
	...
	/*
	 * No "vfragpg volume ..." or "vfragpg domain index ..." from here
	 * on.
	 */
	if (bfp->pvol)
		usage();
	...
	/*
	 * If a file was specified the next argument (if any) is a page
	 * number.
	 */
	if (bfp->type & DMN || bfp->type & SAV) {
		...
		if (bsBfSetAttrp->cloneId != 0) {
			/*
			 * Modify inmem setid to that of original set's
			 * setid.
			 */
...
print_page(bitFileT * bfp, int page, int flg)
{
	...
	if (bfp->type & FLE) {
		...
		bfp->pgNum = page;	/* for routines that expect pgNum to
					 * be set up */
		print_header(bfp);
...
print_fragpage(bitFileT * bfp, fragSlotT * grpPgp, int flgs)
{
	...
	if (flgs & VFLG) {
		...
		if (grpHdrp->version == BF_FRAG_VERSION) {
			for (i = 0; i < BF_FRAG_GRP_SLOTS; i++) {
				/*
				 * Slots not on free list can be anything.
				 * Just test range.
				 */
...
process_summary(bitFileT * bfp, bsBfSetAttrT * bsBfSetAttrp, char *fset, int flgs)
{
	...
	int            *groupPage[BF_FRAG_MAX];	/* group page numbers for
						 * each group type */
	int             totalGroups[BF_FRAG_MAX];	/* total groups of each
							 * group type */
	int             freeFrags[BF_FRAG_MAX];	/* number of free frags in
						 * each group type */
	int             groups;
	bsBfSetAttrT    setAttr;
	int             fileOK;
	...
	*frags = (fragT *) malloc(groups * sizeof(fragT));
	...
	for (grp = 0; grp < groups; grp++) {
		...
		if (bfp->type & FLE) {
			...
			if (ret == BAD) {
				/*
				 * File not saved by nvfragpg. Therefore no
				 * BfSetAttrT.
				 */
				goodFile = FALSE;
				bsBfSetAttrp = NULL;
			...
			grpPgp = (slotsPgT *) PAGE(bfp);	/* not needed. see next
								 * line */
			grpHdrp = (grpHdrT *) grpPgp;
		}
...
print_freelist_heads(bitFileT * bfp, bsBfSetAttrT * bsBfSetAttrp, fragT * frags)
{
	...
	/*
	 * Print "BF_FRAG_ANY" line seperately since "ANY" is longer than
	 * "1K".
	 */
...
{
	int             freeGroups[BF_FRAG_MAX];	/* number of groups with
							 * frag frags */
	int             type;
	int             i = 0;
	     ...
	     int **groupPage, int *totalGroups, int *freeGroups, int groups)
{
	...
	for (type = 0; type < BF_FRAG_MAX; type++) {
		...
		/*
		 * Get the head of the free list from the bsBfSetAttrT
		 * record.
		 */
		...
		while (free != (uint) - 1) {
			...
			if (grp >= groups) {
				/*
				 * No need to printf here. Seen in
				 * get_frag_grp.
				 */
				break;
			}
...
test_saved_slot(bitFileT * bfp, fragSlotT * fragSlotp)
{
	...
	if (((savGrpT *) fragSlotp)->exists == FALSE) {
		...
		/*
		 * If 'exists' is FALSE and the slot is not all zeros (except
		 * saved
		 */
		...
		/*
		 * The 'exists' flag says it was a saved sparse group and it
		 * was
		 */
	...
	for (i = offset; i < BF_FRAG_SLOT_BYTES; i++) {
		if (((char *) fragSlotp)[i] != '\0') {
			/*
			 * Most likely a bad saved file, but I can't tell for
			 * sure.
			 */
			/*
			 * It could be a properly saved corrupted file. Print
			 * the warning
			 */
			/* but return OK. */
			ret = OK;
	...
	/*
	 * There's data where there should be data and zeros where there
	 * should
	 */
...
test_self(uint self, uint32T pgNum)
{
	if (pgNum == (uint32T) - 1) {
		/*
		 * We're looking at a block on disk. We don't know the page
		 * number.
		 */
		return 0;
	}
...
test_freeFrags(grpHdrT * grpHdrp, uint dvn, uint32T pgNum)
{
	...
	if (fragType == BF_FRAG_ANY || fragType >= BFS_FRAG_MAX) {
		/*
		 * BF_FRAG_ANY groups don't change the free list or freeFrags
		 * from
		 */
		/*
		 * what it was before. So freeFrags could be previously valid
		 * number.
		 */
	       ...
	       uint32T pgNum)
{
	...
	if (grpHdrp->firstFrag == 0 && grpHdrp->fragType == BF_FRAG_ANY) {
		/*
		 * The last 2 allocated BF_FRAG_ANY groups may have firstFrag
		 * == 0
		 */
		if (grpHdrp->nextFreeGrp == BF_FRAG_EOG) {
			/*
			 * This is the last BF_FRAG_ANY group on the free
			 * chain.
			 */
		...
		if (bsBfSetAttrp != NULL) {
			...
			if (grpHdrp->nextFreeGrp == lastgrp) {
				/*
				 * Second to the last group. firstFrag == 0
				 * is OK.
				 */
				return 0;
			}
...
test_freeGrp(bitFileT * bfp, uint32T freeGrp, int type, char *head)
{
	...
	if (freeGrp == (uint) - 1) {
		/*
		 * -1 is an OK page in a free list since it is used as the
		 * terminator
		 */
		return 0;
	}
	...
	if (bfp->type & FLE) {
		...
		/*
		 * The other 15 pages in the group will be tested in
		 * get_frag_grps.
		 */
...
test_xxxFreeFrag(grpHdrT * grpHdrp, uint32T pgNum)
{
	...
	if (grpHdrp->version == BF_FRAG_VERSION) {
		...
		/*
		 * lastFreeFrag can be -1 or the new flag BF_FRAG_BAD_GROUP
		 * (1).
		 */
...
test_setId(bfSetIdT testSetid, bfSetIdT refSetId, uint32T pgNum)
{
	...
	if (BS_BFS_EQL(testSetid, refSetId)) {
		...
		/*
		 * Only way for pgNum to be not set is for this to be a -b
		 * request.
		 */
		/*
		 * In that case, there will be no refSetId
		 * (refSetId==nilBfSetId).
		 */
		assert(pgNum != -1);
		fprintf(stderr, "*** Page %u. ", pgNum);
...
test_free_frag_slots(grpHdrT * grpHdrp, uint index, uint dvn, int prtFlg)
{
	...
	if (fragType >= BFS_FRAG_MAX) {
		/*
		 * If the fragType is corrupt we can't say anything about the
		 * slot.
		 */
		return 0;
	}
	...
	if (test_frag_slot(index, grpHdrp, index, dvn, FALSE) == 0) {
		...
		/*
		 * The contents in this slot must point to another slot in
		 * the list.
		 */
		return test_frag_slot(contents, grpHdrp, index, dvn, prtFlg);
	}
