 ...
 * 
 * Input: bfp->pvol - volume on which to find the BMT * Input:
 * bfp->dmn->vols[] filled in * Output: fileTag, setTag (vol 1 defaults for
 * FLE & vol == 0) * OutPut: ppage, pcell * Return: 0 if all went OK. *
...
find_BMT(bitFileT * bfp, int flags)
{
	...
	if (bfp->dmn->vols[bfp->pvol]) {
		...
		/*
		 * bfp->dmn->vols[]->(r)bmt. load_xtnts() will just copy
		 * them.
		 */
		/* This can't fail. */
		assert(ret == OK);
 ...
 * page 0 of the BMT. The -f flag will display the total free mcells in * the
 * BMT (for a suammry of the BMT). *
 * 
 * Input: bfp->dmn *        bfp->pvol - specifies volume to summarize. 0 means
 * all volumes * Output: none * Return: does not return
 */
...
/******************************************************************************/
/*
 * Prints a summary of the RBMT on a volume (or saved file) or all the
 * volumes. * The summary gives the number of pages in the RBMT and the
 * number of free * mcells. *
 * 
 * Input: bfp->pvol - volume to summarize, 0 means all volumes * Input:
 * bfp->dmn->vols[] filled in if DMN * Output: none * Return: does not return
 */
...
print_RBMT_summary(bitFileT * bfp, int flgs)
{
	...
	for (vdi = 1; !stop && vdi < BS_MAX_VDI; vdi++) {
		if (bfp->type & FLE) {
			...
			if (bfp->type & VOL) {
				/*
				 * if pvol set the first time thru (bfp->fd
				 * == -1), then only display summary for one
				 * volume
				 */
				stop = TRUE;
				vdi = bfp->pvol;
			...
			/*
			 * just point to dmn RBMT extents. no possibility of
			 * error.
			 */
			assert(ret == OK);

			/*
			 * The DMN & VOL case haven't read the first page
			 * yet.
			 */
 ...
 * volumes. * The summary gives the number of pages in the BMT and the number
 * of extents * and mcell used to describe the BMT. The number of extents is
 * not given if * bfp is a saved file from a V4 domain. In that case the
 * extents describing * the BMT is in another file. Fo a V3 domain, the
 * number of free mcells in page 0 is reported. If the -f option is set,
 * report the total number * of free mcells in the BMT. *
 * 
 * Input: bfp->pvol - volume to summarize, 0 means all volumes * Input:
 * flgs:FFLG - print total number of BMT free mcells * Input: fflg:V4DMN -
 * bfp is from a V4 domain * Input: bfp->dmn->vols[] filled in if DMN *
 * Output: none * Return: does not return
 */
/* TODO: get_next_mcell checks cell pointer. don't do it here */
print_BMT_summary(bitFileT * bfp, int flgs)
{
	...
	for (vdi = 1; !stop && vdi < BS_MAX_VDI; vdi++) {
		...
		/*
		 * If we have access to the xtnts (V3 BMT or rbmt_present)
		 * count them
		 */
		if (!(bfp->type & FLE && rbmt_present)) {
			 ...
			 * Get the chain mcell from the BSR_XTNTS record in
			 * the primary mcell
			 */
 ...
 * minutes to read. *
 * 
 * Input: bfp->xmap or bfp->fd is set up * Input: flgs:V4DMN - BMT is from a
 * V4 domain * Output: none * Return: never returns
 */
/*
...
print_BMT_freecells(bitFileT * bfp, int flgs)
{
	...
	/*
	 * Print some info about the MCELL_FREE_LIST (-f option). This info
	 * is kept in BMT Page 1 for V3, and BMT Page 0 for Version 4.  (Both
	 * at lbn 48).
	 */
	...
	while (1) {
		/*
		 * V4 domains can have BMT pg 0 on the free list and use pg
		 * -1 as end
		 */
		...
		/*
		 * V3 domains can't have BMT pg 0 on free list. pg == 0 is
		 * the end
		 */
 ...
 * 
 * Input: bfp->xmap or bfp->fd is set up * Input: argv[optind] is the file
 * name * Output: none * Return: never returns
 */
 ...
 * megabytes of data. *
 * 
 * Input: bfp->xmap or bfp->fd is set up * Input: argv, argc, optind just used
 * to check for more (invalid) arguments * Output: none * Return: never
 * returns
 */
 ...
 * are assumed. The block is rounded down to the next multiple of 16 and * is
 * displayed as a page (16 blocks) of BMT. Within the page a particular *
 * mcell can be selected. The size of the volume has been obtained fron * the
 * disklabel not the AdvFS volume size. *
 * 
 * Input: bfp->dmn->vols[bfp->pvol]->blocks - if non zero, used for arg check *
 * Input: argv[optind] = -b <block_number> [cell_number] * Output: none *
 ...
 * 
 * Input: bfp->fd - fd of volume * Input: mcellnum - selected mcell or -1 for
 * all mcells * Input: vflg - verbose display of mcell contents * Output:
 * writes to stdout * Return: OK all went OK *         non zero if lseek or
 * read failed
 */
int
 ...
 * and next mcell. *
 * 
 * Input: bfp->dmn is set, bfp->type is set * Input: argv is a fileset (or
 * tag) and possibly a file path (or tag) * Input: flags:VFLG - display
 * verbose contents of mcell(s) * Output: writes to stdout * Return: never
 * returns
 */
 ...
 * conected mcells. *
 * 
 * Input: bfp defines a (R)BMT file. xmap or fd set up, pvol set up. * Input:
 * longnum - page number * Input: argv, argc, optind = [cell_number [-c]] *
 * Output: writes to stdout * Return: never returns
 */
...
/*****************************************************************************/
/*
 * Prints the page header then prints one or all the mcells. * The -v flag
 * causes all the contents of the header and the mcell(s) to * be printed. *
 * 
 * Input: bfp->type used to print the header, bfp->pgBuf contains the page *
 * Input: vflg:VFLG - print entire header and mcell contents * Output: writes
 * to stdout * Return: OK if all went well *         non zero if read_page()
 * failed
 */
int
 ...
 * 
 * Input: bfp defines the (R)BMT. xmap or fd set. pvol set. * Input: page,
 * cell defines the mcell in the (R)BMT. * Output: writes to stdout * return:
 * OK if all went weel *         non zero if get_chain_mcell or print_chain
 * fails
 */
 ...
 * given mcell. Mcell Id validity is checks at each mcell including the
 * first. *
 * 
 * Input: bfp->dmn set up. bfp->fileTag. dmn->vols[] used to check vdi
 * validity * Input: vn, pn, cn - first in a list of mcells * Output: writes
 * to stdout * Return: OK if all went well *         non zero if list points
 * to a bad vn, pn, cn. Or if read_page fails.
 */
int
 ...
 * 
 * Input: bfp selects the (R)BMT * Input: pn selects the page. * Input:
 * vflg:VFLG - print the entire header contents * Output: writes to stdout *
 * Return: OK if all went well *         non zero if free list has bad data
 * or read_page() fails
 */
int
 ...
 * search for a given fragment in the mcells for a given fileset. * Prints
 * the mcell that conatins the searched for information. * vbmtpg {domain vdi
 * | vol} -s b block [-c] * vbmtpg domain set -s {f <fragid> | F <page>
 * <frag>}          search *
 * 
 * Input: bfp specifies the (R)BMT on a volume for block searchs *        bfp
 * specifies a domain and fileset for frag searches * Input: argv, argc,
 * optind - search type and block or frag * Input: flags:VFLG - print entire
 * contents of found mcell * Output: writes to stdout * Return: never returns
 */
 ...
 * 
 * Input: bfp defines the (R)BMT to search. xmap set up * Input: block - find
 * an extnet with this block * Input: flags:VFLG - print entire contents of
 * found cell *        flags:CFLG - keep on searching after first match found *
 * Output: writes to stdout * Return: never returns
 */
search_block(bitFileT * bfp, char *argv[], int argc, int optind, int flags)
{
	...
	if (ret == ENOENT && bfp->dmn && RBMT_VERSION(bfp->dmn->dmnVers)) {
		...
		/*
		 * Check 'other' BMT only if this is a volume (not a file)
		 * and
		 */
 ...
 * 
 * Input: bfp specifies the domain. bfp->dmn filled in * Input: bfp->setTag
 * specifies the set tag to search * Input: fragid specifies the frag to look
 * for. * Input: flags:VFLG - print entire contents of found mcell * Output:
 * prints to stdout * Return: never returns
 */
 ...
 * 
 * Input: bfp specifies the domain. bfp->dmn filled in * Input: bfp->setTag
 * specifies the set tag to search * Input: flags:VFLG - print entire
 * contents of found mcell * Output: prints to stdout * Return: never returns
 */
 ...
 * 
 * Input: bfp defines the domain and its RBMTs and BMTs *        bfp->pvol - 0
 * means search all volumes, non zero means search one *        flags:VFLG -
 * verbose print mcell contents *        flags:CFLG - continue, keep
 * searching after match found *        flags:RBMT - search RBMT * Output:
 * writes to stdout * Return:
 */
int
 ...
 * 
 * Input: bfp->type used to print cell header * Input: cellp - contenst of
 * cell to search * Input: cn - cell number of cell to search. used to
 * display the contents * Input: bsXA, ndesc - extent array to search *
 * Input: block - look for this block in the extent array * Input: flags:VFLG
 * - print entire cell contents * Input: dmnVersion - passed to print_cell *
 * Output: prints to stdout * Return: OK if bsXA covers block *
 ...
 * specifies the set tag to search * Input: fragid specifies the frag to look
 * for. * Input: flags:VFLG - print entire contents of found mcell * Output:
 * prints to stdout * Return: OK if tag is found *         ENOENT if tag is
 * not found
 */
...
/******************************************************************************/
/*
 * Find the vdi from a file. * If the file is a RBMT just find the vdi in the
 * BSR_VD_ATTR record. * If the file is a BMT file, look for mcell chains
 * whose vdi will indicate * the vdi of this file.
 */
find_n_set_vdi(bitFileT * bfp)
{
	...
	if (version < FIRST_RBMT_VERSION) {
		...
		/*
		 * This may be a BMT file or a RBMT file. Only a RBMT file
		 * will
		 */
