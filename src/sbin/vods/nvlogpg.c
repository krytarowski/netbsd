...
find_LOG(bitFileT * bfp)
{
	...
	int             mc;
	bsMCT          *mcellp;
	extern int      dmnVersion;	/* declared in vods.c; set in
					 * read_rbmt() */
	...
	for (vdi = 0; vdi < BS_MAX_VDI; vdi++) {
		...
		if (bfp->dmn->vols[vdi]) {
			...
			if ((signed) mcellp->tag.num != RSVD_TAGNUM_GEN(vdi, mc)) {
				...
				/*
				 * FIX. "Tag %d is not the tag of a LOG
				 * file.\n",
				 */
				continue;
			}
			if (find_bmtr(mcellp, BSR_XTNTS, (char **) &xtntrp) != OK) {
				fprintf(stderr,
				    ...
				    mc, RBMT_PRESENT ? "RBMT" : "BMT", vdi);
				/*
				 * FIX. "Can't find BSR_XTNTS record for LOG
				 * file.\n"
				 */
				continue;
			}
...
find_log_end_pg(bitFileT * bfp /* log file description */ )
{
	...
	uint32T         lwrPg = 0, uprPg = lastPg, midPg, endPg, pg, ptrdPg = -1;
