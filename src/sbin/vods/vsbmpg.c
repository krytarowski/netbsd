...
/*****************************************************************************/
/*
 * Try to guess the version of the domain for SBM saved files and * corrupted
 * volumes. * If we have a version number from the AdvFS on disk structures
 * use that. * Otherwise, read the SBM pages and look for any non FF and non
 * 00 bytes. * The existance of such a byte is taken to mean that the domain *
 * is version 4 with 1 bit per page allocation. If all the bytes * are 00 or
 * FF then we assume that the domain is version 3.
 */
find_version(bitFileT * bfp)
{
	...
	for (stop = FALSE, vdi = 1; !stop && vdi < BS_MAX_VDI; vdi++) {
		if (bfp->type & FLE) {
			...
			if (savpvol != 0) {
				/*
				 * if pvol set then we only want to know
				 * about one volume
				 */
				stop = TRUE;
				vdi = savpvol;
...
/*****************************************************************************/
/*
 * Input: bfp->pvol - volume on which to find the SBM * Input:
 * bfp->dmn->vols[] filled in
 */
int
...
process_summary(bitFileT * bfp, int flag)
{
	...
	for (vdi = 1; !stop && vdi < BS_MAX_VDI; vdi++) {
		if (bfp->type & FLE) {
			...
			if (savpvol != 0) {
				/*
				 * if pvol set then only display summary for
				 * one volume
				 */
				stop = TRUE;
				vdi = savpvol;
