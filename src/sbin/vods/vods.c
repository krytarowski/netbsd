	    ...
	    int *flagp)
{
	...
	while ((c = getopts(argc, argv, optstr)) != EOF) {
		...
		case 'V':	/* next argument is a volume name */
			...
			if (typeFlg) {	/* temp. later allow multiple -V
					 * volume */
				fprintf(stderr,
		...
		case 'v':	/* verbose */
			*flagp |= VFLG;	/* print all information in the
					 * mcells */
			break;
	...
	/*
	 * Check ret from resolve_name(). Accessability errors are caught
	 * here.
	 */
	/*
	 * AdvFS volume format errors are caught after allowing the -b
	 * option.
	 */
	if (ret != OK && ret != BAD_DISK_VALUE) {
		exit(ret);
	...
	/*
	 * Check if a volume index was supplied with the domain name. If so,
	 * process that volume only.
	 */
	if (bfp->type & DMN &&
	    argc > *optindp &&
		...
		/*
		 * For one-per-volume files (RBMT, BMT, SBM), if the source
		 * is a
		 */
		...
		/*
		 * For one-per-domain files (TAG, LOG), if the source is a
		 * domain,
		 */
		/* the next arg is a vdindex ONLY if the following arg is -b. */
		/*
		 * In that case we are indentifying the volume to look at raw
		 * blocks.
		 */
		/*
		 * Otherwise a number after the domain name identifies a page
		 * number.
		 */
		if (bfp->type & DMN &&
		    argc > optind + 1 &&
...
getopts(int argc, char *const * argv, char const * optstring)
{
	...
	if (*++cp == ':') {
		/*
		 * Option requires a parameter.  There may or may not be
		 * spaces
		 */
...
resolve_name(bitFileT * bfp, char *name, int rawflg)
{
	...
	/*-----------------------------------------------------------------------*/
	/*
	 * third choice is to interpret 'name' as a file, however it must be
	 * REG
	 */
	if (bfp->type & FLE) {
...
#endif
	{
		if (stat_ret != -1 && S_ISDIR(statbuf.st_mode)) {
			...
			bfp->type = SAV;
			strcpy(bfp->dmn->dmnName, name);	/* check for NULL
								 * reseults */
			if (bfp->pvol != 0) {
				bfp->type |= VOL;
			}
...
open_dmn(char *dmnDir, bitFileT * bfp, char *dmnName, int rawflg)
{
	...
	char           *cp;
	char            errstr[80];
	char            dirname[MAXPATHLEN];	/* used to make
						 * /etc/fdmns/<dmn>/<vol>
						 * path */
	char            devname[MAXPATHLEN];	/* used to make domain volume
						 * blk dev name */
	char            rawname[MAXPATHLEN];	/* used to make domain volume
						 * char dev name */
	vdIndexT        vdi;
	struct stat     statBuf;
	dev_t           dev;
	...
	strcpy(bfp->dmn->dmnName, dmnName);	/* FIX. check for NULL
						 * reseults */
	...
	for (dp = readdir(dirStrm); dp != NULL; dp = readdir(dirStrm)) {
		...
		*cp = '\0';
		...
		/*
		 * stat the dir entry. symbolic links stat thru to their
		 * target.
		 */
		if (stat(dirname, &statBuf) == -1) {
			closedir(dirStrm);
		...
		/*
		 * Ignore regular files, eg locks, just pay attention to
		 * block files.
		 */
		if (!S_ISBLK(statBuf.st_mode)) {
			continue;
		}
		dev = statBuf.st_rdev;	/* used to compare to raw dev in
					 * mkraw */
		...
		if (!S_ISLNK(statBuf.st_mode)) {
			fprintf(stderr,
				...
				dirname, dmnDir);
			/*
			 * Even though this is not a symlink, use it as a
			 * volume.
			 */
...
mkraw(const char *blkname, char *rawname, dev_t dev)
{
	...
	/*
	 * Test that the proper naming convention was used, and the
	 * manufactured
	 */
...
/******************************************************************************/
/*
 * Open the volume and fill in the volume specific parts of bfp. * Malloc
 * bfp->dmn->vols[0]. Setup bfp->dmn->vols[0] until the volume index * can be
 * determined then move vols[0] to vols[vdi]. * Setup vols[0]->volName &
 * vols[0]->fd. Fill in the vols[0]->blocks, the * size of the volume. Malloc
 * and fill in vols[0]->rbmt. After the version * of the domain is
 * determined, if the version is < 4, move rbmt to bmt and * leave rbmt =
 * NULL. Read the page at block 32. Find vdi and the domain * version number.
 * For V4 domains, setup vols[vdi]->bmt. * Test the volume AdvFS magic number
 * but just issue a warning if not OK. *
 * 
 ...
 * 
 * Input: dirname = synlink name in /etc/fdmns/<dmn> *        devname = name
 * of special block (character) device to open *        bfp->dmn is malloced *
 * OutPut: malloc and fill in bfp->dmn->vols[vol] (volName, index, fd) *
 * malloc and fill in vols[vol]->rbmt {type, blocks, pvol, ppage, *
 * pcell, xtnts, dmn} *         malloc and fill in vols[vol]->bmt *
 * set pvol. NO, open_dmn doesn't want pvol set * Errors: diagnostic output
 * to stderr for all errors * Return: OK if all went well *         ENOENT -
 * no such volume *         EBUSY - block device is mounted *         EACCES
 * - no permissions to access volume *         ENOMEM - malloc failed *
...
open_vol(bitFileT * bfp, char *devname, vdIndexT * vdip)
{
	...
	/*
	 * TODO. use different test for OK volume based just on BMT page 0
	 * data.
	 */
	if ((ret = test_advfs_magic_number(volFd, devname)) != OK) {
		/* Return ENOMEM or BAD_DISK_VALUE */
		/*
		 * BAD_DISK_VALUE is OK if we are looking at a block on the
		 * disk
		 */
 ...
 * 
 * Input: fd - lseek and read this file * Input: devname used only to print
 * warning * Output: none * Errors: diagnostic output to stderr for all
 * errors * Return: OK if all went well *         ENOMEM - if malloc of
 * buffer to read page failed *         BAD_DISK_VALUE - bad magic number
 */
static int
...
get_vol_blocks(volT * vol)
{
	...
	if (is_lsm(vol) == FALSE) {
		/*
		 * If that fails then get the volume size from the disk
		 * label.
		 */
		read_disk_label(vol);
	}
...
/*****************************************************************************/
/*
 * Given page 0 of the (R)BMT, look at the mcell with the vdIndex. * Check
 * the tag and set tag of the mcell. Look for BSR_VD_ATTR record. * Check the
 * vdIndex. *
 * 
 * Input: Global dmnVersion has been set *        bmtpgp - pointer to (R)BMT
 * page 0 *        bfp->dmn->vols[] - filled in. used to check vol previously
 * found * Output: vdip - returns volume index * Errors: diagnostic output to
 * stderr for all errors * Return: OK if all went well *
	...
	if (bsVdAttrp->vdBlkCnt != bfp->dmn->vols[0]->blocks) {
		if (bfp->dmn->vols[0]->blocks == 0) {
			...
			if (bfp->dmn->vols[0]->blocks == 0xffffffff) {
				/*
				 * blocks has been squashed by get_vol_blocks
				 * to fit
				 */
				fprintf(stderr,
...
/*****************************************************************************/
/*
 * Load the RBMT extent map, following the chain of RBMT pages. * Each page
 * of the RBMT file contains, in cell 27, a BSR_XTNT * record whose extent
 * describes the next page of the RBMT. The * chain and next also point to
 * the next RBMT page. * Called from open_vol. *
 * 
 * Input: Global dmnVersion set and >= 4 * Input: bfp->dmn->vols[] *
 * bfp->pcell set to 0 *        bfp->pgBuf already read, page 0 of RBMT *
 * Output: bfp->xmap - filled in * Errors: diagnostic output to stderr for
 * all errors * Return: OK if all went well *         ENOMEM - malloc fails *
...
load_rbmt_xtnts(bitFileT * bfp)
{
	...
	/*
	 * vdi from the chain mcell will be zero if there are no
	 * BSR_XTRA_XTNTS
	 */
	...
	while (vdi > 0) {
		...
		if (ret == ENOENT) {
			/*
			 * No BSR_XTRA_XTNTS record in this cell. End of the
			 * chain.
			 */
			break;
		}
		...
		/*
		 * Test additional restriction on page. One addtional mcell
		 * per page.
		 */
 ...
 * 
 * Input: bfp - BMT bitfile *        bfp->pcell - set to primary mcell of BMT
 * (0) *        bfp->pgBuf - contains page 0 (block 32) * Output: bfp->xmap,
 * pages - filled in * Errors: diagnostic output to stderr for all errors *
 * Return: OK if all went well *         BAD_DISK_VALUE for bad mcell
 * contents *         errors from load_bsr_xtnts, load_xtra_xtnts
 */
static int
load_V3bmt_xtnts(bitFileT * bfp)
{
	...
	while (vdi > 0) {
		...
		/*
		 * test for valid page; not restricted to page 0 as of
		 * Version 4
		 */
		if (bfMCId.page != 0) {
 ...
 * dmn->vols[pgVol]->fd - file descriptor of opened volume * Output:
 * bfp->pgBuf may be malloc'd *         bfp->pgBuf filled * Errors:
 * diagnostic output to stderr for all errors * Return: OK if all went well *
 * ENOMEM if malloc failed *         BAD_DISK_VALUE - lseek or read fails
 */
static int
 ...
 * 
 * V3 non reserved files & striped files have no extents in BSR_XTNTS. * The
 * first extents are in BSR_SHADOW_XTNTS. The next extents are in *
 * BSR_XTRA_XTNTS. *
 * 
 ...
 * bfp->dmn->vols[]->(r)bmt. *
 * 
 * Input: bfp->fileTag - use RBMT vs BMT for mcells *        bfp->pvol,
 * bfp->ppage, bfp->pcell - primary mcell in (R)BMT * Output: bfp->xmap.xtnts
 * loaded *         bfp->pages set * Errors: diagnostic output to stderr for
 * all errors * Return: OK if all went well *         ENOMEM if malloc of
 * xtnts fails *         BAD_DISK_VALUE - extents or mcells not as expected /*
...
load_xtnts(bitFileT * bfp)
{
	...
	/*
	 * really V4 since this is xtnts in prim. remember stripe doesn't do
	 * this
	 */
...
un_stripe(bitFileT * bfp, segMapT * segMapp)
{
	...
	while (page < bfp->pages) {
		...
		for (segidx = 0; segidx < segMapp->segCnt; segidx++) {
			...
			/*
			 * This is not required if read_page handled
			 * consecutive holes.
			 */
...
load_clone_xtnts(bitFileT * bfp)
{
	...
	if (bfp->setp->origSetp == NULL) {
		/*
		 * This file is not in a clone fileset. Return w/ orig
		 * extents
		 */
		return OK;
	}
	...
	if (ret != OK) {
		...
		/*
		 * else the clone has no meta data. just read the original
		 * BSR_ATTR
		 */
	}
...
merge_clone_xtnts(bitFileT * cloneBfp, bitFileT * origBfp)
{
	...
	while (page < cloneBfp->pages) {
		if (cloneXi < cloneBfp->xmap.cnt &&
			...
			/*
			 * Use the clone extent. We use the orig extent only
			 * where the
			 */
			/* clone has a hole. */
		...
		/*
		 * The clone extent at cloneXi is a hole or the end of clone
		 * extents.
		 */
		...
		if (origXi >= origBfp->xmap.cnt - 1) {
			...
			/*
			 * Copy the rest of the clone's extents and we're
			 * done.
			 */
		...
		if (origBfp->xmap.xtnt[origXi + 1].page <= page) {
			/*
			 * The orig does not overlap the clone hole. It is
			 * completely
			 */
			/*
			 * before the hole. Move the orig extent pointer
			 * forward.
			 */
			/*
			 * This orig extent maps pages already merged into
			 * clone.
			 */
			origXi++;
		...
		if (nextOrigPage < nextClonePage) {
			/*
			 * We used the entire orig extent. Move on to the
			 * next.
			 */
			origXi++;
			assert(origXi < origBfp->xmap.cnt);
			...
			/*
			 * We passed the entire clone hole. Move on to the
			 * next extent.
			 */
			cloneXi++;
			page = nextClonePage;
			...
			/*
			 * The orig extent is used up, the clone hole is
			 * passed. Move on.
			 */
			cloneXi++;
			origXi++;
 ...
 * BSR_XTNTS record. Load the extent from the record. Reserved files * and
 * non striped files in a V4 domain have extents in the primary * mcell. V3
 * nonreserved files and striped files do not have extents * in the primary
 * mcell. In files that have extents in the primary mcell * the BSR_XTNTS
 * record tell how many mcells have extenst. Thsi is used * to malloc the
 * extent array in bfp. *
 * 
 * must have tag (file or set) since ppage==0 could be reg file in V4 * Input:
 * bfp->fileTag - use RBMT for negative fileTag, else use BMT *
 * bfp->dmn, bfp->dmn->vols[pvol]->(r)bmt - extents of (R)BMT *
 * bfp->pvol, bfp->ppage, bfp->pcell - location of prim mcell * Output:
 * bfp->xmap.xtnts[0] filled in *         bfp->pages set. *         cvdi,
 * cmcid - location of chain mcell *         idxp - next location in
 * bfp->xmap.xtnts[] to fill *         mcCntp - number of extent mcells in
 * the chain * Errors: diagnostic output to stderr for all errors * Return:
 * OK if all went well *         BAD_DISK_VALUE - extents or mcells not as
 * expected *         ENOMEM - malloc failed
 */
static int
 ...
 * 
 * Input: mcellp - mcell contents with BSR_XTNTS record * Output:
 * bfp->xmap.xtnts[0] filled in *         bfp->pages set. *         idxp -
 * set to 1 if BSR_XTNTS had one extent *         cvdi, cmcid - pointer to
 * chain mcell on disk *         mcCntp - number of extent mcells in the
 * chain * Errors: diagnostic output to stderr for all errors * Return: OK if
 * all went well *         BAD_DISK_VALUE if bad mcell contents. *
 * ENOMEM - malloc failed
 */
static int
 ...
 * 
 * Input: *vdip, *pagep, cellp = shadow cell * Output: bfp->xmap.xtnts filled
 * in *         bfp->pages set. *         *idxp = next index in xtnts to fill
 * in. *         *vdip, *pagep, cellp = first xtra cell *mcCntp = number of
 * extent mcells in the chain * Errors: diagnostic output to stderr for all
 * errors * Return: OK if all went well *         BAD_DISK_VALUE for bad
 * mcell contents *         ENOMEM if malloc of extent array fails
 */
static int
 ...
 * Find the xtra mcell and the BSR_XTRA_XTNTS record. Load the extents * into
 * bfp->xmap.xtnts. * Since some files can have many extra mcells, a progress
 * report is * printed every 100 mcells read. *
 * 
 * Input: idx = index in bfp->xmap to start filling at *        vol, pn, cn =
 * first xtra mcell *        stflg - striped file, non XTRA cell after chain
 * of XTRA cells is OK *        mcCnt - number of extent mcells in the chain *
 * Output: bfp->xmap.xtnts filled in *         bfp->pages set. * Errors:
 * diagnostic output to stderr for all errors * Return: OK if all went well *
 * BAD_DISK_VALUE if mcell contents are bad *         ENOMEM - malloc failed
 */
static int
 ...
 * bfp->xmap.xtnts. Test the mcell contents. Set bfp->pages. * If we didn't
 * malloc enough when the BSR_XTNTS/BSR_SHADOW_XTNTS record * was read, get
 * some more now. This only happens if the mcell count * in the
 * BSR_XTNTS/BSR_SHADOW_XTNTS record is bad. *
 * 
 * Input: idxp - index in extent array to start loading *        bsMCp - mcell
 * contents *        stflg - striped file, expect non  XTRA cell after chain
 * of XTRA cells * Output: bfp->xmap.xtnts filled in *         bfp->pages set *
 * idxp - incremented *         vnp, bfMCIdp - next xtra mcell pointer *
 * Errors: diagnostic output to stderr for all errors * Return: OK if all
 * went well *         ENOENT if no BSR_XTRA_XTNTS record found. Used for
 * stripes *         BAD_DISK_VALUE if mcell contents bad *         ENOMEM if
 * malloc fails
 */
static int
 ...
 * 
 * Input: bsXA = array of extents from disk *        cnt = number of entries
 * in bsXA *        *idxp = start filling bfp->xmap here * Output:
 * bfp->xmap.xtnts (partially) filled in *         bfp->pages - one page past
 * last page (so far). *         *idxp = one page past last page filled. *
 * Errors: diagnostic output to stderr for all errors * Return: none
 */
	...
	if (*idxp == 0) {
		...
		if (bsXA[0].bsPage == lastp) {
			if (bfp->xmap.xtnt[*idxp - 1].blk == XTNT_TERM) {
				/*
				 * Collapse the two identical entries at the
				 * end of one
				 */
				/*
				 * extent map and the beginning of the next
				 * extent map.
				 */
				(*idxp)--;
...
/*****************************************************************************/
/*
 * If this is the first volume opened, malloc and init bfp->dmn. * Malloc and
 * setup bfp->dmn->vols[vdi]. *
 * 
 * Input: devname - device to open * Output: bfp->type set to VOL *
 * bfp->dmn - may be malloced *         bfp->fd - set *         vdi - volume
 * index found by reading volume * Errors: diagnostic output to stderr for
 * all errors * Return: OK if all went well *         ENOENT - volume doesn't
 * exist *         EBUSY - block device is mounted *         EACCES -
 * insufficient permissions *         BAD_DISK_VALUE - bad magic or
 * unexpected disk value *         ENOMEM - malloc failed
 */
static int
...
/*****************************************************************************/
/*
 * Given a file name, open the file and test its length. * All saved reserved
 * files are multiples of 8K. Only the metadata * of a frag file is saved.
 * The metadata is 1K per 16 pages of frag file. * So saved frag files should
 * be multiples of 1K and we fake the original * frag file lenght by
 * multiplying each 1K by 16. *
 * 
 * Input: name - file pathname to open *        statp - stats of file *
 * bfp->fileTag - used to test length * Output: bfp->type set to FLE *
 * bfp->fd set to fd of file *         bfp->pages set to number to 8K pages
 * in file *         bfp->fileName set * Errors: diagnostic output to stderr
 * for all errors * Return: OK if all went well *         ENOENT - no such
 * file or bad size *         EACCES - can't access "name" due to permissions
 */
static int
resolve_fle(bitFileT * bfp, char *name, struct stat * statp)
{
	...
	if (bfp->fileTag < 0) {
		...
		/*
		 * FIX. BF_FRAG_SLOT_BYTES (1024) needs
...
open_savset_vol(bitFileT * bfp, char *name)
{
	...
	int             ret;
	int             vol;
	int             attr_mcell = BFM_RBMT_EXT;	/* mcell with vol
							 * attributes */
	ssize_t         read_ret;
 ...
 * Read page 0 of (R)BMT on each volume and find BSR_XTNTS record * in
 * primary mcell for TAG file. If it has extents, we found it. *
 * 
 * Input: bfp->dmn->vols[] * Output: bfp->xmap.xtnts loaded *
 * bfp->pvol, bfp->ppage, bfp->pcell - set to prim mcell of TAG file *
 * Errors: diagnostic output to stderr for all errors * Return: OK if all
 * went well *         BAD_DISK_VALUE if TAG file not found
 */
int
		...
		if (!recFound) {
			...
			*memDmnMAttrp = *bufDmnMAttrp;
			...
			if (bufDmnMAttrp && bufDmnMAttrp->seqNum > maxSeqNum) {
				/*
				 * Found another BSR_DMN_MATTR record. Save
				 * the latest.
				 */
				maxSeqNum = bufDmnMAttrp->seqNum;
				*memDmnMAttrp = *bufDmnMAttrp;
 ...
 * 
 * Input: argv, argc, optindp - filset name/tag and file name/tag * Output:
 * bfp->xmap.xtnts filled in *         bfp->pages filed in *
 * bfp->pvol, bfp->ppage, bfp->pcell - set to prim mcell of file *
 * optindp points to next input arg * Errors: diagnostic output to stderr for
 * all errors * Return: OK if all went well *         BAD_SYNTAX - too many
 * args, args not the right kind (int vs string) *         EINVAL - bad arg
 * range *         ENOENT - no such set or file *         BAD_DISK_VALUE -
 * bad extents or tag file or other unexpected value *         ENOMEM -
 * malloc failed
 */
int
...
/*****************************************************************************/
/*
 * Given the definition of the root TAG file and the name or tag of a file
 * set, * find the primary mcell and load the extents of the fileset. * The
 * fileset can be specified by name or tag. * "bfp" is filled in to represet
 * the fileset's TAG file. *
 * 
 * Input: argv, argc, optindp - fileset name or tag *        rootTag -
 * definition of root TAG file * Output: bfp->xmap.xtnts - filled in for
 * fileset *         bfp->pages - set *         bfp->pvol, bfp->ppage,
 * bfp->pcell - primary mcell of fileset *         bfp->setTag, bfp->fileTag
 * - set * Errors: diagnostic output to stderr for all errors * Return: OK if
 * all went well *         BAD_SYNTAX - command line syntax errors *
 * EINVAL - out of range numbers *         ENOENT - no such set *
...
get_set(bitFileT * rootTag, char *argv[], int argc, int *optindp, bitFileT * bfp)
{
	...
	if (strcmp(argv[*optindp], "-T") == 0) {
		...
		if (ret != OK) {
			/*
			 * FIX. let find_rec print for BAD_DISK. print here
			 * for ENOENT
			 */
			fprintf(stderr,
 ...
 * with the fileset name in the BSR_BFS_ATTR record. If it matches load * the
 * fileset extents. *
 * 
 * Input: name = fileset name *        rootBfp - root TAG file * Output:
 * bfp->xmap.xtnts, bfp->pages - set *         bfp->setName - set *
 * bfp->pvol, ppage, pcell - set to primary mcell of fileset TAG file *
 * bfp->setTag - set to root set tag (-2) *         bfp->fileTag - set to
 * fileset tag in root TAG file * Errors: diagnostic output to stderr for all
 * errors * Return: OK if all went OK *         ENOENT if no such fileset *
 * BAD_DISK_VALUE - bad extents or tag file or directory *         ENOMEM -
 * malloc failed
 */
static int
 ...
 * 
 * Input:  rootBfp - root TAG file *         tagnum - tag number of original
 * set *         bfp - clone set * Output: bfp->origSetp - setup * Errors:
 * diagnostic output to stderr for all errors * Return: OK if all went OK *
 * ENOENT if no such fileset *         BAD_DISK_VALUE - bad extents or tag
 * file or directory *         ENOMEM - malloc failed
 */
static int
...
/******************************************************************************/
/*
 * Find the primary mcell and load the extents of a file given the fileset
 * TAG * file and a file path name or a file tag. The path name is relative
 * to the * root of the fileset. Ie it does not begin with a /, nor does it
 * include * the mount point (especially since the fileset is probably not
 * mounted). *
 * 
 * Input: argv, argc, optindp - file pathname or file tag number *
 * setBfp is the fileset TAG file * Output: bfp->pvol, bfp->ppage, bfp->pcell
 * - set to primary mcell of file *         bfp->xtnts - filled in for file
 * in fileset * Errors: diagnostic output to stderr for all errors * Return:
 * Ok if all went well *         BAD_SYNTAX for cammand line syntax errors *
 * EINVAL for out of range numbers *         ENOENT - no such file *
 * BAD_DISK_VALUE - bad extents, tag file, directory, etc *         EINVAL -
 * tag number out of range *         ENOMEM - malloc failed
 */
...
/*****************************************************************************/
/*
 * Given a root or fileset TAG file and a tag, fill in bfp and load the
 * extents. *
 * 
 * Input: tagBfp - TAG file *        tagnum - tag of file in the TAG file *
 * Output: bfp - file to init * Errors: status return only. No diagnostic
 * printf * Return: OK if all goes well *         ENOENT if tag not in use in
 * TAG file *         BAD_DISK_VALUE - bad extents *         ENOMEM - malloc
 * failed
 */
int
 ...
 * Reads the fileset tag file to find the root dir prime mcell. * Loads the
 * root tag extents and reads the directory. * For each component of the
 * path: find the tag in the directory, find * the prime mcell in the fileset
 * tag file and load the extents. * At the last component return the prime
 * mcell. Does not load extents of * found file. *
 * 
 * Input: tagBfp is tag file *        path is fileset relative path * Output:
 * bfp->pvol, bfp->ppage, bfp->pcell - primary mcell of found file * Errors:
 * diagnostic output to stderr for all errors * Return: OK if file found in
 * set *         ENOENT - file does not exist *         BAD_DISK_VALUE - bad
 * extents, tag file or directory *         EINVAL - badly formed .tags
 * pathname *         ENOMEM - malloc failure
 */
#define TAG3 98
...
find_file_by_name(bitFileT * tagBfp, char *path, bitFileT * bfp)
{
	...
	char           *path_end;
	int             ret;
	bitFileT       *dirBfp = &dirBf;	/* Used for each dir in the
						 * path */
...
/*****************************************************************************/
/*
 * The next path component in not in the directory. * It is in the .tags
 * directory. It is a tag. * Called from find_file_by_name *
 * 
 * Input: component - path name in .tags dir *        dirBfp - ??? *
 * tagBfp - tag file description * Output: bfp - primary mcell filled in *
 * Errors: diagnostic output to stderr for all errors * Return: OK -
 * component found and resolved *         ENOENT - no such component *
 * BAD_SYNTAX - not a number in .tags dir *         EINVAL - invalid number
 * in .tags
 */
static int
...
/*****************************************************************************/
/*
 * Given the tag of a reserved file, find the primary mcell. * The tag gives
 * the volume and the reserved file. * Reads every volume and lloks for
 * BSR_VD_ATTR record with matching vdIndex. * Called from
 * find_prim_from_dot_tags. *
 * 
 * Input: bfp->dmn->vols[]->(r)bmt - (R)BMT files on volumes in domain *
 * tag - tag of reserved file * Output: bfp->fd - fd of found volume *
 * bfp->pvol, bfp->ppage, bfp-pcell = primary mcell of reserved file *
 * bfp->xmap setup * Errors: diagnostic output to stderr for all errors *
 * Return: OK if reserved file found *         EINVAL for bad tag number *
 * ENOENT for no such reserved file tag found *         errors from
 * read_vol_at_blk
 */
static int
find_rsrv_prim_from_tag(bitFileT * bfp, bfTagT tag)
{
	...
	for (i = 0; i < BS_MAX_VDI; i++) {
		if (bfp->dmn->vols[i]) {
			...
			if (RBMT_PRESENT) {
				/*
				 * reserved files use the rbmt for their
				 * extents
				 */
			...
			bfp->pvol = i;
			/*
			 * FIX. notice this only works for BMT, not any other
			 * rsv file.
			 */
 ...
 * find and test the chain pointer. * Called from nvbmtpg.c &
 * load_xtnts_from_prim. *
 * 
 * Input: bfp->pvol, bfp->ppage, bfp->pcell - point to the primary mcell *
 * Output: vol, mcidp - point to the chain mcell * Errors: diagnostic output
 * to stderr for all errors * Return: OK if all went well *
 * BAD_DISK_VALUE for bad mcell contents
 */
/*
 * TODO: set setTag *       if filetag == 0 set it
 */
int
 ...
 * 
 * Input: bfp->type - saved file or disk *        bfp->fileTag - for disk,
 * select RBMT or BMT for mcells *        bfp->dmn->vols[*vdi]->(r)bmt -
 * pointers to BMT file *        vdip, mcidp - point to current mcell *
 * Output: bfp->pgBuf - filled with next mcell page *         vdip, mcidp -
 * point to next mcell * Errors: diagnostic output to stderr for all errors *
 * Return: OK if all went well *         BAD_DISK_VALUE if cell has bad
 * contents
 */
...
get_next_mcell(bitFileT * bfp, vdIndexT * vdip, bfMCIdT * mcidp)
{
	...
	if (cellp->nextVdIndex && (cellp->nextMCId.page != mcidp->page)) {
		...
		/*
		 * save these values; cellp points to new page after
		 * read_page
		 */
...
/*****************************************************************************/
/*
 * Given the tag file for the fileset and a directory in the fileset and * a
 * path conponent (ie directory entry), find the tag of the component. * On
 * by one, read the directory pages and for each dir entry, test the *
 * component name. If found, return the tag of the component in bfp->fileTag. *
 * 
 * Input: dirbfp is set up for the dir to search *        component - file or
 * next dir to find in dirbfp * Output: dirbfp->fileTag -  component tag *
 * Errors: diagnostic output to stderr for all errors * Return: OK if
 * component found *         TAG3 if found conponent is tag 3 (.tags) *
 * ENOENT if component does not exist *         BAD_DISK_VALUE for mangled
 * directory entries
 */
static int
 ...
 * 
 * Input: bfp->dmn - defines the domain and BMT extents *        bfp->fileTag
 * - negative tag uses RBMT (if V4 domain) *        bfp->pvol, bfp->ppage,
 * bfp->pcell - primary mcell *        recpa - address of pointer to found
 * record * Output: recpa - pointer to the found record. NULL is not found *
 * Errors: diagnostic output to stderr for all errors * Return: OK if record
 * found *         BAD_DISK_VALUE for bad mcell contents *         ENOENT if
 * record is not found *         errors from find_bmtr
 */
...
/*****************************************************************************/
/*
 * Change the number in the character string to a long. * Accept octal, hex
 * or decimal. If the string has any bad numbers, return * an error without
 * outputing a number. *
 * 
 * Input: character string cp with octal, decimal or hexadecimal number. *
 * Output: long at "num" iff conversion is OK. * Errors: status return only.
 * No diagnostic printf * Return: OK if converstion went well *
...
/*****************************************************************************/
/*
 * Prints a header for the page of data displayed. * Prints a one line
 * description of the source of the formated data. * For a domain it prints
 * the domain name, the volume and the block number. * Then it prints the
 * bitfile name (BMT, SBM, etc) and the bitfile page number. * For a volume
 * it prints the volume name, block, bitfile name and page. * For a saved
 * file it prints the file name and the page number. * If no page number is
 * specified (block on disk is specified) then just * print the given block
 * number. * A double line "====" precedes the description and a single line
 * follows it. *
 * 
 * DOMAIN "---" VDI # ("<vol>") LBN #  <file> PAGE # * VOLUME "---" (VDI #)
 * LBN #  <file> PAGE # * FILE "---"  PAGE # *
 * 
 * <file> = BMT, SBM, LOG, ROOT TAG, "file_set" TAG, "file_set" FRAG, "file" *
 * 
 * DOMAIN "---"  VDI # ("<vol>")  LBN # * VOLUME "---" (VDI #)  LBN # * VOLUME
 * "---"  LBN # *
 * 
 * SAVE SET "---"  VDI #  SAVE FILE "---"  PAGE # *
 * 
 * Input: bfp->type = DMN, VOL or FLE *        bfp->fileTag - selects file
 * name *        bfp->pgVol - if DMN or VOL then VDI to print *
 * bfp->dmn->vols[bfp->pgVol]->volName - if DMN or VOL then volume name *
 * bfp->pgLbn - if != -1 then LBN to print *        bfp->pgNum - if != -1
 * then file page number to print * Output: prints volume page header *
 * Errors: no errors * Return: none
 */
void
...
/*****************************************************************************/
/*
 * Prints groups of 4 bytes as an int until < 4 bytes left to print, * then
 * prints bytes, all in hex. *
 * 
 * Input: pdata - data to be displayed. No alignment required *        size -
 * number of bytes to displayed * Output: print to stdout * Errors: no errors *
 * Return: none
 */
 ...
 * 
 * Input: bfp->type *        bfp->pages - if bfp->type == FLE or SAV *
 * bfp->xmap - if bfp->type == VOL or DMN * Output: none * Errors: return
 * status only. no errors * Return: TRUE if page is mapped otherwise FALSE
 */
int
...
/*****************************************************************************/
/*
 * Read a page from the bitFile into the bitFile page buffer. * Maintain a
 * simple one page cache. If pgVol & pgLbn as calculated from page * and xmap
 * are aready set in bfp then pgBuf already contains the page. *
 * 
 * input: bfp->xmap hold extents for file *        bfp->type determines
 * whether to use xtnts or fd *        bfp->fd - read from this fd is type is
 * FLE or VOL *        bfp->pgVol, pgNum, pgLbn - if same as requested, don't
 * read again *        page is bitFile page to read * output: bfp->pgBuf - a
 * page of data is loade here *         bfp->pgBuf - if NULL, malloc a page
 * buffer *         bfp->pgVol, pgNum, pgLbn - set to reflect where pgBuf
 * read from * Errors: diagnostic output to stderr for all errors * return OK
 * - if read succeeds *        BAD_DISK_VALUE - block is not mapped or volume
 * does not exist *        ENOMEM - malloc failed
 */
...
read_page(bitFileT * bfp, int page)
{
	...
	if (bfp->type & FLE || bfp->type & SAV) {
		/*
		 * working with save file;  page # is real offset into the
		 * file
		 */
		...
		while ((idx > 0) &&
			...
			/*
			 * is this a test that the page is off the end?
			 * return err
			 */
			assert(idx != lastidx);
		}
...
/*****************************************************************************/
/*
 * Test the BMT page. * Test nextfreeMCId, nextFreePg, freeMcellCnt, pageId,
 * megaVersion * Example message: * open_vol: Corrupted RBMT page 5 (lbn 564)
 * on volume 2 (/dev/disk/dsk8c). * Bad pageId (23). *
 * 
 * Input:  bfp->pgBuf - page to check *         bfp->pgNum - page number *
 * bfp->pages - number of pages in the (R)BMT if known * Output: none *
 * Errors: diagnostic output to stderr for all errors * Return: OK if all
 * went well *         BAD_DISK_VALUE if a value is bogus
 */
static int
...
/*****************************************************************************/
/*
 * Test fields in an mcell. * tag and setTag in mcell must match fields
 * bitFileT. *
 * 
 * Input:  bfp->fileTag, bfp->setTag *         bsMCp - pointer to mcell
 * contents *         func_name - routine that called. If NULL then no
 * fprintf. *         vdi, bfMCId - used to ID mcell in diag printf * Output:
 * none * Errors: diagnostic output to stderr for all errors if func_name not
 * NULL * Return: OK if all went well *         BAD_DISK_VALUE if the mcell
 * has errors
 */
static int
 ...
 * 
 * Input:  bfMCId.page, bfMCId.cell - tested *         vdi - volume mcell
 * lives on *         bfp->dmn->vols[vdi]->[r]bmt - test page agaisnt this
 * BMT *         func_name - used in diag printf. if NULL, no printf *
 * Output: none * Errors: diagnostic output to stderr for all errors if
 * func_name not NULL * Return: OK if all went well *         BAD_DISK_VALUE
 * if the mcell id is out of range
 */
static int
 ...
 * 
 * Input:  bfp->dmn->vols[] - array of valid volumes in domain *         vdi -
 * volume index to test *         func_name - used in diag printf. no printf
 * if NULL * Output: none * Errors: diagnostic printf if func_name not NULL *
 * Return: OK if vdi passes all tests *         OUT_OF_RANGE if vdi is too
 * large *         BAD_DISK_VALUE if vdi is not in this domain
 */
static int
 ...
 * boundary. The number of blocks in a volume may not be set up. If not * it
 * will be zero. In order to test extents, blk -1 & -2 are "OK". *
 * 
 * Input: bfp->dmn->vols[vdi]->blocks - blocks in the volume *        vdi -
 * volume number on which to test block *        blk - block to test *
 * Output: none * Errors: status return only. No diagnostic printf * Return:
 * OK if block passes all tests *         BAD_DISK_VALUE if block is not a
 * multiple of 16 *         OUT_OF_RANGE if block is too large for the volume
 */
static int
