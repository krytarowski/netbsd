...
int dmn_locked = 0;
int numSets = 0;
int maxSets = 100;		/* Initial value - dynamically increased if
				 * more needed */
...
main(int argc, char *argv[])
{
	...
	for (i = 0; i < vdIndexCnt; i++) {
		...
		if (sts == EOK) {
			...
			/* check entire free mcell list starting at the vd's
			 * free mcell listhead */
		} else {
			errcnt++;
		}
...
)
{
	...
	int next_free_page, free_list_head_page, free_list_terminator;
	off_t lseek_status;
...
)
{
	...
	case BSR_ATTR:
		...
		if (recType == BSR_XTNTS) {
			...
			/* If this is not a reserved file or a file which
			 * contains extent information in the primary mcell,
	...
	for (i = 0; i < (bsXACnt - 1); i++) {
		...
		if (firstIndex == lastIndex) {
			...
			if (file_tags)
				filetags[firstIndex] = (u32T) mcell->tag.num;
			if (set_tags)
				settags[firstIndex] = (u32T) mcell->bfSetTag.num;
		} else {
			/*
	                 * First cell.
	                 */
...
)
{
	...
	if (setHdrTbl != NULL) {
		...
		free(setHdrTbl);
		/* Update memory used: (newSetHdrTbl size = (setHdrCnt + 1) *
		 * sizeof (setHdrT)) - (setHdrTbl size = setHdrCnt * sizeof
		 * (setHdrT)) =  sizeof (setHdrT) */
...
reallocate_bf_mounted_array(void)
{
	...
	if (mounted != NULL) {
		maxSets = (maxSets * 150) / 100;	/* increase allocation
							 * by 50% */
	}
	/*
...
)
{
	...
	do {
		...
		if (ret == EOK) {
			...
			if (n < 2)
				n = 2;	/* minimum needed if root - see below */
...
)
{
	...
	struct stat stats;
	slotsPgT grppg;
	int sts, freeFrags, grpFType, no_errors_found_in_this_chain;
	void *xtntmap;
	...
	for (idx = 0; idx < INODE_TBL_SZ; idx++) {
		...
		while (curEnt != NULL) {
			...
			if (curEnt->dirLinkCnt == 0 && curEnt->ino != 2) {
				...
				/* TODO : create a real directory entry and
				 * fix link count */
				...
				/* TODO : This most likely is an active mount
				 * point  */
...
)
{
	...
	while (curEnt != NULL) {
		if (curEnt->ino == ino) {
			...
			/* the inode already exists; just increment the link
			 * count */
			curEnt->dirLinkCnt++;
			return;
		}
...
)
{
	...
	xtntMapT *xtntmap;
	mlExtentDescT xtnt;
	char corruptpath[MAXPATHLEN], fileName[MAXPATHLEN];
	long fragPg;
	...
	if (dirStats.st_dev != mountDev) {
		if ((!active) || (verbose)) {
			...
			/* Removed the check in lost files for mountpoints, so
			 * the following error message is currently not
			 * needed. fprintf( stderr, catgets(catd, 1, 222, "%s:
			 * mount points will appear as lost files\n"), Prog );
			 * 
			 */
			fflush(stderr);
		}
	...
	while ((dirEntry = readdir(dp)) != NULL) {
		...
		if (lstat(pathName, &stats) < 0) {
			...
			pathName[len + 1] = '\0';	/* chop off the file
							 * name */
			continue;
		}
		...
		case S_IFREG:
			...
			if (fileFd < 0) {
				...
				if (sts != EOK) {
					...
					/* read the corresponding frag group
					 * header */
