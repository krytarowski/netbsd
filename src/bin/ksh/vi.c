...
#define is_cmd(c)	(classify[(c)&0x7f]&(M_|E_|C_|U_))OB
...
static void
restore_edstate(struct edstate *old, struct edstate *new)
{
	memcpy(new->cbuf, old->cbuf, old->linelen);
	new->linelen = old->linelen;
	new->cursor = old->cursor;
