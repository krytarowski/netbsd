...
/*
 * Check that the caller knows about the on-disk structures on this system.
 * 
 * Since there is no locking done around checking and setting of
 * checked_on_disk_version, this library, or at least those calls that use
 * this variable, are not thread-safe. There is no problem with this right
 * now due to the check-once way it is used.
 */
mlStatusT
...
/*
 * Packs the parameters from user mode and calls msfs_syscall to transfer
 * them to kernel mode
 */
...
)
{
	...
	if (volInfop)
		*volInfop = buf.getVolParams.volInfo;
...
msfs_get_dmn_vol_list(
		      ...
		      int volIndexArrayLen,	/* in - number of ints in
						 * array */
		      u32T volIndexArray[],	/* out - list of volume
						 * indices */
...
msfs_dmn_init(
	      ...
	      mlServiceClassT tagSvc,	/* in - tag directory service
					 * attributes */
...
)
{
	...
	do {
		...
		/*
		 * The current dir is set to the parent dir; do the parent
		 * next
		 */
...
islsm(char *lsm_path, int *volsize)
{
	...
	/*
	 * Now check if the filesystem size has already been set.  If not we
	 * need to retrieve and set that information.
	 */
...
get_dev_size(
	     ...
	     char *devType,	/* in - optional device type ("rz55", etc or
				 * NULL) */
...
)
{
	...
	if (islsm64(devName, &lsmDevSize)) {
		...
		if (lsmDevSize == 0) {
			...
			iDevSize = (unsigned int) lsmDevSize;
		...
		if (strchr("abcdefgh", partName) == 0) {
			/*
			 * This should never happen unless the partition
...
int 
unlock_file(
	    int lockHndl,	/* in - lock handle (really just the file
				 * desc) */
	    int *error		/* out - errno if return -1 */
)
{
...
advfs_post_user_event(char *evname, advfs_ev advfs_event, char *Prog)
{
	...
	/*
	 * EvmEventPost waits for daemon response before returning, due to
	 * EvmRESPONSE_WAIT in EvmConnCreate
	 */
