...
)
{
	...
	*status = EOK;
	...
	if (xtnt_use != 1) {
		...
		/*
		 * first get the xtntCnt so we can allocate the total number
		 * of extents for the merged extent map correct during the
		 * next call to advfs_get_bkup_xtnt_map
		 */
	...
	if (xtnt_use != 1) {
		...
		/*
		 * check to make sure the maps have not grown since called
		 * above
		 */
